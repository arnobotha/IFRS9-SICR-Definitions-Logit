...
# - Define model form
inputs_chosen <- SICR_target ~ InterestRate_Margin + BalanceLog + pmnt_method_grp + slc_acct_pre_lim_perc_imputed + 
...
optimal.cutpoint.GenYouden <- optimal.cutpoints(X = "Prob_chosen_2c_ii", status = "SICR_target", tag.healthy = 0, # these are the negatives
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_2c_ii, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_2c_ii, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.2283887
...
pack.ffdf(paste0(genPath, "performance_measures_", SICR_label), performance_measures_2c_ii); gc()
