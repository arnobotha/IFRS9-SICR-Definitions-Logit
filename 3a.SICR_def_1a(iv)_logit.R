# ============================== SICR-MODELLIING ===============================
# SICR-model for definition 1a(iv) using Logistic Regression (LR)
# ------------------------------------------------------------------------------
...
ggplot(plot.data_SICR_rates, aes(x=Date, y=Proportion, colour=Prop)) + 
  ...
  labs(y="SICR-incidence rates for definition with parameters k=12, d=1, and s=1", x= "Time") + 
...
# Age_Adj, Balance, Redraw_Ind (so we need to remove Redraw_Amt as well), FurtherLoan_Amt (so we need to remove FurtherLoan_Ind as well), 
# AgeToTerm and BalanceToTerm
...
inputs_account3 <- SICR_target_event ~ Term + Receipt_Inf + InterestRate_Margin + LN_TPE 
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_2) # 63.94%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_2) # 63.74%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_3) # 60.07%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_3) # 59.97%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_1) # 81.57%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_1) # 81.91%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_2) # 81.56%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_2) # 81.91%

# Slightly higher AUC on the training dataset using the imputed variables.
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_3) # 73.90% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_3) # 74.11%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_4) # 73.90% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_4) # 74.11%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_1) # 58.52% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_1) # 58.52%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_2) # 56.98% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_2) # 56.99%

# More than a 1% reduction in the AUC
...
# M_Inflation_Growth, M_Inflation_Growth_1, M_Inflation_Growth_3, M_Inflation_Growth_7, M_Inflation_Growth_10, M_Inflation_Growth_11, 
# M_Inflation_Growth_12, M_Inflation_Growth_Vol_2, M_Inflation_Growth_Vol_5, M_Inflation_Growth_Vol_7, M_Inflation_Growth_Vol_8,
# M_Inflation_Growth_Vol_9 and M_Inflation_Growth_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_3) # 57.21% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_3) # 57.22%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_4) # 56.85% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_4) # 56.85%

# Less than a 1% reduction in the AUC
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_5) # 59.89% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_5) # 59.89%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_6) # 59.51% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_6) # 59.50%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_7) # 58.66% 
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_8) # 57.73% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_8) # 57.73%
...
# M_RealGDP_Growth and M_RealGDP_Growth_12 are significant
...
# M_RealIncome_Growth, M_RealIncome_Growth_11, M_RealIncome_Growth_12, M_RealIncome_Growth_Vol_2 and M_RealIncome_Growth_Vol_10 are significant
...
# M_RealIncome_Growth, M_RealIncome_Growth_2, M_RealIncome_Growth_11 and M_RealIncome_Growth_12 are significant

# Model with all the significant MVs
inputs_macro <- SICR_target_event ~ M_Repo_Rate + M_Repo_Rate_2 + M_Repo_Rate_4 + M_Repo_Rate_6 + M_Repo_Rate_8 + M_Repo_Rate_10 + M_Repo_Rate_12 +
                                    M_Inflation_Growth + M_Inflation_Growth_1 + M_Inflation_Growth_10 + M_Inflation_Growth_11 +
                                    M_Inflation_Growth_12 + M_DTI_Growth + M_DTI_Growth_11 + M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_1 +
                                    M_Emp_Growth_8 + M_Emp_Growth_9 + M_Emp_Growth_11 + M_Emp_Growth_12 + M_RealGDP_Growth + M_RealGDP_Growth_1 +
                                    M_RealIncome_Growth + M_RealIncome_Growth_2 + M_RealIncome_Growth_11 + M_RealIncome_Growth_12


# --- 4.5 Combined information
# Analyse the all the significant variables from the modelling themes
# inputs_chosen <- SICR_target_event ~ Term + Receipt_InfLog + BalanceLog + BalanceToTerm + InterestRate_Margin + LN_TPE +
                                     ...
                                     # M_DTI_Growth + M_DTI_Growth_12 + M_Emp_Growth + M_RealIncome_Growth
                                     # AgeToTerm + M_Repo_Rate_4 + M_Repo_Rate_6 + M_Repo_Rate_8 + M_Repo_Rate_12 + M_Inflation_Growth_11 +
                                     # M_Inflation_Growth_12 + M_DTI_Growth_11 + M_Emp_Growth_1 + M_Emp_Growth_8 + M_Emp_Growth_11 + M_Emp_Growth_12 +
                                     # M_Emp_Growth_9 + M_Inflation_Growth_1 + M_RealGDP_Growth + M_RealGDP_Growth_1 + Age_Adj + M_Repo_Rate_2
                                     # M_RealIncome_Growth_2 + M_RealIncome_Growth_11 + M_RealIncome_Growth_12
                                      
# logit_model_chosen <- glm(inputs_chosen, data=dat_SICR_def_train_s, family="binomial")
# summary(logit_model_chosen)

# - Compute the AUC
# dat_SICR_def_train_s[, Prob_chosen_1a_iv := predict(logit_model_chosen, newdata = dat_SICR_def_train_s, type="response")] 
# dat_SICR_def_valid_s[, Prob_chosen_1a_iv := predict(logit_model_chosen, newdata = dat_SICR_def_valid_s, type="response")]

# auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_chosen_1a_iv) 
# auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_chosen_1a_iv) 

# NOTE: Including the volatility variables - Report the AUC for the validation dataset
# Iteration 1's AUC: 85.23% | Iteration 2's AUC: 85.23% | Iteration 3's AUC: 85.22% | Iteration 4's AUC: 85.23% | Iteration 5's AUC: 85.21% |
# Iteration 6's AUC: 85.21%

# NOTE: Including the volatility variables - Report the AUC for the validation dataset
# Iteration 1's AUC: 85.20% | Iteration 2's AUC: 85.18% | Iteration 3's AUC: 85.18% | Iteration 4's AUC: 85.18% | Iteration 5's AUC: 85.15%
...
inputs_all_mc1 <- SICR_target_event ~ Term + Receipt_InfLog + BalanceLog + BalanceToTerm + InterestRate_Margin +
                                      ...
                                      M_Repo_Rate + M_Repo_Rate_10 + M_Inflation_Growth + M_Inflation_Growth_10 +
                                      M_DTI_Growth + M_DTI_Growth_12 + M_Emp_Growth + M_RealIncome_Growth 
                                      # Age_Adj + M_Repo_Rate_2 +  M_RealIncome_Growth_2 + M_RealIncome_Growth_11 + M_RealIncome_Growth_12
...
# Iteration 1: Age_Adj, TimeInPerfSpell, as well as all of the macroeconomic variables
# Remove Age_Adj and M_Repo_Rate_2
# Iteration 2: All of the macroeconomic variables (to be expected)
# Remove M_RealIncome_Growth_2 and  M_RealIncome_Growth_11
# Iteration 3: Macroeconomic variable (to be expected)
...
table(datSICR$SICR_target) %>% prop.table() # 93.98% Non-SICR vs 6.02% SICR
...
datSICR_valid[, Prob_chosen_1a_iv := predict(logit_model_chosen, newdata = datSICR_valid, type="response")]
datSICR_smp[, ExpProb := predict(logit_model_chosen, newdata = datSICR_smp, type="response")]

# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_1a_iv) # 84.95%
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_1a_iv) # 84.82%
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 84.91%
...
ggplot( data=datSICR_valid, aes(x=Prob_chosen_1a_iv)) + theme_bw() + 
...
optimal.cutpoint.GenYouden <- optimal.cutpoints(X = "Prob_chosen_1a_iv", status = "SICR_target", tag.healthy = 0, # these are the negatives
...
datSICR_train[, Pred_chosen_1a_iv := ifelse(Prob_chosen_1a_iv >= logistic_cutoff, 1, 0)]
datSICR_valid[, Pred_chosen_1a_iv := ifelse(Prob_chosen_1a_iv >= logistic_cutoff, 1, 0)]
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_1a_iv, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_1a_iv, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
...
datSICR_valid[, SICR_predict_variance := sd(Prob_chosen_1a_iv), by=list(LoanID)]
standard_deviation <- round(mean(datSICR_valid$SICR_predict_variance, na.rm=T)*100, digits=1)

# - Confusion matrix
conf_mat <- datSICR_valid[, list(TN=sum(ifelse(SICR_target == 0 & Pred_chosen_1a_iv == 0, 1, 0)),
                                 FP=sum(ifelse(SICR_target == 0 & Pred_chosen_1a_iv == 1, 1, 0)),
                                 TP=sum(ifelse(SICR_target == 1 & Pred_chosen_1a_iv == 1, 1, 0)),
                                 FN=sum(ifelse(SICR_target == 1 & Pred_chosen_1a_iv == 0, 1, 0)))]
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.11630094
...
performance_measures_1a_iv <- data.frame(SICR_definition = paste0(SICR_label, "_logit"),
                                         d=p.d, s=p.s, k=p.k,
...
pack.ffdf(paste0(genPath, "performance_measures_", SICR_label), performance_measures_1a_iv); gc()
