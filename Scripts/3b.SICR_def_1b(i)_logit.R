# ============================== SICR-MODELLIING ===============================
# SICR-model for definition 1b(i) using Logistic Regression (LR)
# ------------------------------------------------------------------------------
...
ggplot(plot.data_SICR_rates, aes(x=Date, y=Proportion, colour=Prop)) + 
  ...
  labs(y="SICR-incidence rates for definition with parameters k=3, d=1, and s=2", x= "Time") + 
...
inputs_account3 <- SICR_target_event ~ Term + Receipt_Inf + Redraw_Ind + Redrawn_Amt + InterestRate_Margin + FurtherLoan_Ind + FurtherLoan_Amt + LN_TPE + BalanceToTerm

logit_model_account3 <- glm(inputs_account3, data=dat_SICR_def_train_s, family="binomial")
summary(logit_model_account3)
# FurtherLoan_Ind is not significant; however, this is paired with FurtherLoan_Amt. FurtherLoan_Amt is significant, therefore, keep FurtherLoan_Ind

# - Remove the insignificant variable from the log-transform variable theme and analyse again
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_3) # 61.97%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_3) # 61.75%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_4) # 66.80%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_4) # 66.60%
...
# All the variables are significant. However, the slc_acct_arr_dir_3MISSING_DATA group from slc_acct_arr_dir is not significant (p-value of 11.72%)
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_1) # 92.01%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_1) # 92.13%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_2) # 91.99%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_2) # 92.11%

# - Slightly higher AUC on the training dataset using the imputed variables. Look at the p-values again
summary(logit_model_delinq1)
summary(logit_model_delinq2)
# Since all the variables are significant for the value indicator group - keep
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_3) # 76.51% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_3) # 76.49%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_4) # 76.51% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_4) # 76.49%

# - The AUC-values are the same. Look at the p-values and standard errors
summary(logit_model_behav3)
summary(logit_model_behav4)
# p-values and standard errors are equal
# use the value indicator variables to be consistent with the choice from the delinquency theme
...
# M_Repo_Rate, M_Repo_Rate_10, M_Repo_Rate_12, M_Repo_Rate_Vol_3, M_Repo_Rate_Vol_10 and M_Repo_Rate_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_1) # 60.42% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_1) # 60.43%
...
# M_Repo_Rate, M_Repo_Rate_2, M_Repo_Rate_10 and M_Repo_Rate_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_2) # 60.33% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_2) # 60.34%
...
# M_Inflation_Growth, M_Inflation_Growth_1, M_Inflation_Growth_3, M_Inflation_Growth_10, M_Inflation_Growth_11, M_Inflation_Growth_12,
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_3) # 60.57% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_3) # 60.59%
...
# M_Inflation_Growth and M_Inflation_Growth_1 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_4) # 60.16% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_4) # 60.18%
...
# M_DTI_Growth_9, M_DTI_Growth_11, M_DTI_Growth_12, M_DTI_Growth_Vol_2 and M_DTI_Growth_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_5) # 61.96% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_5) # 61.98%
...
# M_DTI_Growth_8, M_DTI_Growth_9, M_DTI_Growth_11 and M_DTI_Growth_12 are significant
# Would need to include the unlagged variable as well in the final model
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_6) # 61.81% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_6) # 61.83%
...
# M_Emp_Growth, M_Emp_Growth_8, M_Emp_Growth_9, M_Emp_Growth_11, M_Emp_Growth_12, M_Emp_Growth_Vol_11 and M_Emp_Growth_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_7) # 60.39% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_7) # 60.41%
...
# M_Emp_Growth_8, M_Emp_Growth_9, M_Emp_Growth_11 and M_Emp_Growth_12 are significant
# Would need to include the unlagged variable as well in the final model
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_8) # 58.91% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_8) # 58.92%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_9) # 61.68% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_9) # 61.69%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_10) # 60.36% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_10) # 60.37%
...
# M_RealIncome_Growth_11, M_RealIncome_Growth_12, M_RealIncome_Growth_Vol_2 and M_RealIncome_Growth_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_11) # 61.50% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_11) # 61.52%
...
# M_RealIncome_Growth, M_RealIncome_Growth_1, M_RealIncome_Growth_3, M_RealIncome_Growth_9, M_RealIncome_Growth_11 and M_RealIncome_Growth_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_12) # 59.46% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_12) # 59.47%

# The reduction in AUC is more significant than the previous variables, but the standard errors are big for the volatility variables

# Model with all the significant MVs
inputs_macro7 <- SICR_target_event ~ M_Repo_Rate + M_Repo_Rate_2 + M_Repo_Rate_10 + M_Repo_Rate_12 +  M_Inflation_Growth + M_Inflation_Growth_1 + M_DTI_Growth +
                                     M_DTI_Growth_8 + M_DTI_Growth_9 + M_DTI_Growth_11 + M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_8 + M_Emp_Growth_9 +
                                     M_Emp_Growth_11 + M_Emp_Growth_12 + M_RealGDP_Growth + M_RealGDP_Growth_11 + M_RealGDP_Growth_12 + M_RealIncome_Growth +
                                     M_RealIncome_Growth_1 + M_RealIncome_Growth_3 + M_RealIncome_Growth_9 + M_RealIncome_Growth_11 + M_RealIncome_Growth_12


# --- 4.5 Combined information
# Analyse the all the significant variables from the modelling themes
inputs_chosen <- SICR_target_event ~ Age_Adj + Term + BalanceLog + LN_TPE + BalanceToTerm + InterestRate_Margin + TimeInPerfSpell + 
                                     PerfSpell_Num + g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed:value_ind_slc_acct_roll_ever_24 +
                                     slc_pmnt_method + slc_acct_pre_lim_perc_imputed:value_ind_slc_acct_pre_lim_perc + 
                                     M_Repo_Rate + M_Repo_Rate_10 + M_DTI_Growth + M_DTI_Growth_12
                                     # Receipt_InfLog + M_Repo_Rate_2 + M_DTI_Growth_11 + M_Emp_Growth + M_Emp_Growth_8 + M_Emp_Growth_9 + M_Emp_Growth_11 + M_Emp_Growth_12 +
                                     # M_RealGDP_Growth + M_RealGDP_Growth_11 + M_RealGDP_Growth_12 + M_RealIncome_Growth_3 + M_RealIncome_Growth_11 + M_RealIncome_Growth_12
                                     # PrevDefaults + M_Repo_Rate_12 + M_RealIncome_Growth + M_RealIncome_Growth_1 + M_RealIncome_Growth_9
                                     # M_Inflation_Growth + M_Inflation_Growth_1 + M_DTI_Growth_8 + M_DTI_Growth_9

logit_model_chosen <- glm(inputs_chosen, data=dat_SICR_def_train_s, family="binomial")
summary(logit_model_chosen)

# NOTES: Significance of the variables
# Iteration 1
# The following variables are significant in no particular order:
# Age_Adj, Term, BalanceLog, LN_TPE, BalanceToTerm, InterestRate_Margin, TimeInPerfSpell, PerfSpell_Num, g0_Delinq, slc_acct_arr_dir_3, slc_pmnt_method, M_Repo_Rate, M_Inflation_Growth_1,
# M_DTI_Growth, M_DTI_Growth_8, M_DTI_Growth_9, slc_acct_roll_ever_24_imputed:value_ind_slc_acct_roll_ever_24, and slc_acct_pre_lim_perc_imputed:value_ind_slc_acct_pre_lim_perc
# The following variables have p-values greater than 5% but smaller than 20%:
# PrevDefaults, M_Repo_Rate_10, M_Repo_Rate_12, M_Inflation_Growth_1, M_DTI_Growth_12, M_RealIncome_Growth, M_RealIncome_Growth_1
# Iteration 2
# The following variables are significant in no particular order:
# Age_Adj, Term, BalanceLog, LN_TPE, BalanceToTerm, InterestRate_Margin, TimeInPerfSpell, PerfSpell_Num, g0_Delinq, slc_acct_arr_dir_3, slc_pmnt_method, M_Repo_Rate, M_Inflation_Growth_1,
# M_DTI_Growth, M_DTI_Growth_8, M_DTI_Growth_12, slc_acct_roll_ever_24_imputed:value_ind_slc_acct_roll_ever_24, and slc_acct_pre_lim_perc_imputed:value_ind_slc_acct_pre_lim_perc
# The following variables have p-values greater than 5% but smaller than 20%:
# M_Repo_Rate_10 and M_DTI_Growth_9
# Iteration 3
# The following variables are significant in no particular order:
# Age_Adj, Term, BalanceLog, LN_TPE, BalanceToTerm, InterestRate_Margin, TimeInPerfSpell, PerfSpell_Num, g0_Delinq, slc_acct_arr_dir_3, slc_pmnt_method, M_Repo_Rate, M_Repo_Rate_10,
# M_DTI_Growth, M_DTI_Growth_8, M_DTI_Growth_12, slc_acct_roll_ever_24_imputed:value_ind_slc_acct_roll_ever_24, and slc_acct_pre_lim_perc_imputed:value_ind_slc_acct_pre_lim_perc
# The following variables have p-values greater than 5% but smaller than 20%:
# M_DTI_Growth_9
# Choose the 2 M_DTI_Growth variables with the smallest p-values, which are: M_DTI_Growth and M_DTI_Growth_12
# Iteration 4
# The following variables are not significant:
# The Suspense group from slc_pmnt_method (p-value of 25.50%)

# - Compute the AUC
dat_SICR_def_train_s[, Prob_chosen_1b_i := predict(logit_model_chosen, newdata = dat_SICR_def_train_s, type="response")] 
dat_SICR_def_valid_s[, Prob_chosen_1b_i := predict(logit_model_chosen, newdata = dat_SICR_def_valid_s, type="response")]

auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_chosen_1b_i) 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_chosen_1b_i) 

# Report the final AUC: 91.61%

# Report the AUC for the validation dataset
# Iteration 1's AUC: 93.50% | Iteration 2's AUC: 93.51% | Iteration 3's AUC: 93.51% | Iteration 4's AUC: 93.51%

# - Include only the numerical variables to analyse possible multicollinearity
inputs_all_mc1 <- SICR_target_event ~ Age_Adj + Term + BalanceLog + BalanceToTerm + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num + g0_Delinq + 
...
table(datSICR$SICR_target) %>% prop.table() # 95.23% Non-SICR vs 4.77% SICR
...
datSICR_valid[, Prob_chosen_1b_i := predict(logit_model_chosen, newdata = datSICR_valid, type="response")]
datSICR_smp[, ExpProb := predict(logit_model_chosen, newdata = datSICR_smp, type="response")]

# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_1b_i) # 93.50%
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_1b_i) # 93.77%
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 93.59%
...
ggplot( data=datSICR_valid, aes(x=Prob_chosen_1b_i)) + theme_bw() + 
...
optimal.cutpoint.GenYouden <- optimal.cutpoints(X = "Prob_chosen_1b_i", status = "SICR_target", tag.healthy = 0, # these are the negatives
...
datSICR_train[, Pred_chosen_1b_i := ifelse(Prob_chosen_1b_i >= logistic_cutoff, 1, 0)]
datSICR_valid[, Pred_chosen_1b_i := ifelse(Prob_chosen_1b_i >= logistic_cutoff, 1, 0)]
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_1b_i, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_1b_i, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
...
datSICR_valid[, SICR_predict_variance := sd(Prob_chosen_1b_i), by=list(LoanID)]
standard_deviation <- round(mean(datSICR_valid$SICR_predict_variance, na.rm=T)*100, digits=1)

# - Confusion matrix
conf_mat <- datSICR_valid[, list(TN=sum(ifelse(SICR_target == 0 & Pred_chosen_1b_i == 0, 1, 0)),
                                 FP=sum(ifelse(SICR_target == 0 & Pred_chosen_1b_i == 1, 1, 0)),
                                 TP=sum(ifelse(SICR_target == 1 & Pred_chosen_1b_i == 1, 1, 0)),
                                 FN=sum(ifelse(SICR_target == 1 & Pred_chosen_1b_i == 0, 1, 0)))]
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.10787923
...
(g <- ggplot(port.aggr, aes(x=Date, y=EventRate, group=Type)) + theme_minimal() + 
    ...
    annotate(geom="text", x=as.Date("2015-12-31"), y=port.aggr[Date >= "2012-12-31" & Type=="a_Actual", mean(EventRate)]*2,
...
performance_measures_1b_i <- data.frame(SICR_definition = paste0(SICR_label, "_logit"),
                                        d=p.d, s=p.s, k=p.k,
...
pack.ffdf(paste0(genPath, "performance_measures_", SICR_label), performance_measures_1b_i); gc()
