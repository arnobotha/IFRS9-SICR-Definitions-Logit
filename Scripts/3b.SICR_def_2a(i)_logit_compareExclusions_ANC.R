# ============================== SICR-MODELLIING ===============================
# Comparative study of default exclusion on SICR-model using definition 2a(i)
# Ancillary script.
# ------------------------------------------------------------------------------
...
# Major agreement between SICR and Default when both equals 0 (93.7%),
#   though there are some illogical cases, e.g., SICR = 0 but Default = 1 (1.2%)
# [POST HOC]: Fixed
...
# Major agreement between SICR and Default when both equals 0 (94.5%),
#   though there are some illogical cases, e.g., SICR = 0 but Default = 1 (<0.9%)
# [POST HOC]: Fixed
...
# No illogical cases (e.g., g0_Delinq > 1 while SICR=0). SICR=1 coincides mostly with g0_Delinq=2 (<0.8%) and g0_Delinq>=3 (3.7%).
# Considerable mass at g0_Delinq = 1 while SICR=0 (5.2%), though this is understandable given d=2 in definition 2a(i).
...
table(datSICR_withDefs$SICR_target) %>% prop.table() # 6% SICR-prevalence
table(datSICR_withDefs[DefaultStatus1==0,SICR_target]) %>% prop.table() # 1.4% SICR-prevalence
table(datSICR_smp$SICR_target) %>% prop.table() # 1.4% SICR-prevalence
