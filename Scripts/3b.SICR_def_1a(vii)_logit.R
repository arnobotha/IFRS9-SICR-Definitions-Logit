# ============================== SICR-MODELLIING ===============================
# SICR-model for definition 1a(vi) using Logistic Regression (LR)
# ------------------------------------------------------------------------------
...
# Number of impacted observations: 43.370%
...
ggplot(plot.data_SICR_rates, aes(x=Date, y=Proportion, colour=Prop)) + 
  ...
  labs(y="SICR-incidence rates for definition with parameters k=36, d=1, and s=1", x= "Time") + 
...
# Balance, Redraw_Ind (so we need to remove Redraw_Amt as well), FurtherLoan_Amt (so remove FurtherLoan_Ind too), LN_TPEWHL and 
...
# Term and AgeToTerm are significant

# - Remove the insignificant variables from the "raw" variable theme and analyse again
inputs_account3 <- SICR_target_event ~ Age_Adj + Term + Receipt_Inf + InterestRate_Margin 
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_3) # 60.63%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_3) # 60.66%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_4) # 64.09%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_4) # 64.07%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_1) # 74.93%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_1) # 75.50%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_2) # 74.92%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_2) # 75.50%

# Slightly higher AUC on the train an dtest dataset using the imputed variables.
# slc_acct_arr_dir_3MISSING_DATA is insignificant for the value indicator variables.
# Conclusion: Use the value imputed variables
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_3) # 69.01% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_3) # 69.30%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_4) # 69.01% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_4) # 69.30%
...
# M_Repo_Rate, M_Repo_Rate_2, M_Repo_Rate_4, M_Repo_Rate_10, M_Repo_Rate_12, M_Repo_Rate_Vol_3 and M_Repo_Rate_Vol_11 are significant
...
# M_Repo_Rate, M_Repo_Rate_2 and M_Repo_Rate_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_1) # 51.91% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_1) # 51.89%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_2) # 51.61% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_2) # 51.60%

# RESULT: Around a 0.3% reduction in AUC 
...
# Same as with the volatility variables
...
# M_DTI_Growth, M_DTI_Growth_1 and M_DTI_Growth_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_5) # 53.64% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_5) # 53.63%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_6) # 53.48% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_6) # 53.47%

# RESULT: Around a 0.1% reduction in AUC
...
# M_Emp_Growth, M_Emp_Growth_11, M_Emp_Growth_12 and M_Emp_Growth_Vol_12 are significant
...
# M_Emp_Growth and M_Emp_Growth_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_7) # 53.24% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_7) # 53.23%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_8) # 52.34% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_8) # 52.33%
...
# M_RealGDP_Growth and M_RealGDP_Growth_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_9) # 53.82% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_9) # 53.81%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_10) # 53.11% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_10) # 53.10%

# RESULT: Around a 0.7% reduction in AUC
...
# M_RealIncome_Growth_Vol_11 and M_RealIncome_Growth_Vol_12 are significant
...
# M_RealIncome_Growth and M_RealIncome_Growth_12 are significant

# Model with all the significant MVs
inputs_macro <- SICR_target_event ~ M_Repo_Rate + M_Repo_Rate_2 + M_Repo_Rate_12 + M_Inflation_Growth + M_DTI_Growth + M_Emp_Growth + M_Emp_Growth_12 +
...
# inputs_chosen <- SICR_target_event ~ Receipt_InfLog + BalanceLog + InterestRate_Margin + TimeInPerfSpell +
                                     # PerfSpell_Num + g0_Delinq + PrevDefaults + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + 
                                     # slc_pmnt_method + slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Repo_Rate_2 + 
                                     # M_Inflation_Growth + M_DTI_Growth
                                     # LN_TPE + M_Repo_Rate_12 + M_Emp_Growth + M_Emp_Growth_12 + M_RealGDP_Growth + M_RealGDP_Growth_12 + 
                                     # M_RealIncome_Growth + M_RealIncome_Growth_12 + Age_Adj
                                      
# logit_model_chosen <- glm(inputs_chosen, data=dat_SICR_def_train_s, family="binomial")
# summary(logit_model_chosen)

# - Compute the AUC
# dat_SICR_def_train_s[, Prob_chosen_1a_viii := predict(logit_model_chosen, newdata = dat_SICR_def_train_s, type="response")] 
# dat_SICR_def_valid_s[, Prob_chosen_1a_viii := predict(logit_model_chosen, newdata = dat_SICR_def_valid_s, type="response")]

# auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_chosen_1a_viii) 
# auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_chosen_1a_viii) 

# Report the AUC for the validation dataset
# Iteration 1's AUC: 79.21% | Iteration 2's AUC: 79.17% | Iteration 3's AUC: 79.16% | Iteration 4's AUC: 79.16% | Iteration 5's AUC: %

# Note: With the volatility variables - Report the AUC for the validation dataset
# Iteration 1's AUC: 79.21% | Iteration 2's AUC: 79.19% | Iteration 3's AUC: 79.19% | Iteration 4's AUC: 79.18% | Iteration 5's AUC: 79.17%

# - Analyse possible multicollinearity
inputs_all_mc1 <- SICR_target_event ~ Receipt_InfLog + BalanceLog + InterestRate_Margin + TimeInPerfSpell +
...
# Iteration 1: Age_Adj, TimeInPerfSpell and the macroeconomic variables (to be expected)
# Remove Age_Adj
# Iteration 2: Just the repo rate variables (to be expected)
...
table(datSICR$SICR_target) %>% prop.table() # 94.79% Non-SICR vs 5.21% SICR
...
summary(logit_model_chosen)
# Most variables are statistically significant, except: M_Inflation_Growth
# [Ad hoc] PD ratio is not statistically significant (p-value of 0.37508 and standard error of 0.0005304) as well as inflation growth

# - Score data using fitted model
datSICR_train[, Prob_chosen_1a_vii := predict(logit_model_chosen, newdata = datSICR_train, type="response")] 
datSICR_valid[, Prob_chosen_1a_vii := predict(logit_model_chosen, newdata = datSICR_valid, type="response")]
datSICR_smp[, ExpProb := predict(logit_model_chosen, newdata = datSICR_smp, type="response")]

# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_1a_vii) # 78.41% (before and after)
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_1a_vii) # 79.02% vs 79.01%
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 78.60% (before and after)
...
ggplot( data=datSICR_valid, aes(x=Prob_chosen_1a_vii)) + theme_bw() + 
...
datSICR_train[, Pred_chosen_1a_vii := ifelse(Prob_chosen_1a_vii >= logistic_cutoff, 1, 0)]
datSICR_valid[, Pred_chosen_1a_vii := ifelse(Prob_chosen_1a_vii >= logistic_cutoff, 1, 0)]
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_1a_vii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_1a_vii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
...
datSICR_valid[, SICR_predict_variance := sd(Prob_chosen_1a_vii), by=list(LoanID)]
standard_deviation <- round(mean(datSICR_valid$SICR_predict_variance, na.rm=T)*100, digits=1)

# - Confusion matrix
conf_mat <- datSICR_valid[, list(TN=sum(ifelse(SICR_target == 0 & Pred_chosen_1a_vii == 0, 1, 0)),
                                 FP=sum(ifelse(SICR_target == 0 & Pred_chosen_1a_vii == 1, 1, 0)),
                                 TP=sum(ifelse(SICR_target == 1 & Pred_chosen_1a_vii == 1, 1, 0)),
                                 FN=sum(ifelse(SICR_target == 1 & Pred_chosen_1a_vii == 0, 1, 0)))]
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.09756648
...
(g <- ggplot(port.aggr, aes(x=Date, y=EventRate, group=Type)) + theme_minimal() + 
    ...
    #annotations
    annotate(geom="text", x=as.Date("2014-12-31"), y=port.aggr[Date >= "2012-12-31" & Type=="a_Actual", mean(EventRate)]*1.9,
             label=paste0("'MAE between '*italic(A[t])*' and '*italic(B[t])*': ", sprintf("%.2f", diag.Act_ExpProb),"%'"),
             family=chosenFont, size=3, parse=T) + 
    annotate(geom="text", x=as.Date("2014-12-31"), y=port.aggr[Date >= "2012-12-31" & Type=="a_Actual", mean(EventRate)]*1.8,
...
performance_measures_1a_vii <- data.frame(SICR_definition = paste0(SICR_label, "_logit"),
                                          d=p.d, s=p.s, k=p.k,
...
pack.ffdf(paste0(genObjPath, "performance_measures_", SICR_label), performance_measures_1a_vii); gc()
