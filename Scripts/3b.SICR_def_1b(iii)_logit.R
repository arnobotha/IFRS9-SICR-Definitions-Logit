# ============================== SICR-MODELLIING ===============================
# SICR-model for definition 1b(iii) using Logistic Regression (LR)
# ------------------------------------------------------------------------------
...
ggplot(plot.data_SICR_rates, aes(x=Date, y=Proportion, colour=Prop)) + 
  ...
  labs(y="SICR-incidence rates for definition with parameters k=9, d=1, and s=2", x= "Time") + 
...
# Balance, FurtherLoan_Amt (so remove FurtherLoan_Ind as well), AgeToTerm and BalanceToTerm
...
# Age_Adj and Redraw_Ind are not significant, but Redraw_Amt is, so keep
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_2) # 64.10%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_2) # 64.00%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_4) # 60.30%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_4) # 59.88%
...
# TimeInPerfSpell is not significant, but given the significance in 1a, keep
# PrevDefaults not significant - remove

inputs_delinq2 <- SICR_target_event ~ TimeInPerfSpell + PerfSpell_Num + g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed 

logit_model_delinq2 <- glm(inputs_delinq2, data=dat_SICR_def_train_s, family="binomial")
summary(logit_model_delinq2)
# All the variables except TimeInPerfSpell are significant

# - Value indicator variable
inputs_delinq3 <- SICR_target_event ~ TimeInPerfSpell + PerfSpell_Num + g0_Delinq + slc_acct_arr_dir_3 + PrevDefaults + 
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_2) # 84.26%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_2) # 84.21%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_4) # 84.26%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_4) # 84.21%

# - The variables are more significant for the value indicator variables - keep
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_3) # 74.68% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_3) # 74.65%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_4) # 74.68% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_4) # 74.65%
...
# M_Repo_Rate, M_Repo_Rate_2, M_Repo_Rate_4, M_Repo_Rate_6, M_Repo_Rate_8, M_Repo_Rate_9, M_Repo_Rate_10, M_Repo_Rate_11, 
# M_Repo_Rate_Vol_3, M_Repo_Rate_Vol_5, M_Repo_Rate_Vol_7, M_Repo_Rate_Vol_10 and M_Repo_Rate_Vol_12 are significant
...
# M_Repo_Rate, M_Repo_Rate_2, M_Repo_Rate_3, M_Repo_Rate_4, M_Repo_Rate_6, M_Repo_Rate_8 and M_Repo_Rate_10 are significant

# - Compute the AUC to inspect the difference in AUC
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_1) # 60.06% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_1) # 60.07%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_2) # 59.02% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_2) # 59.05%

# Just a little more than a 1% reduction
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_3) # 59.40% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_3) # 59.44%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_4) # 58.98% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_4) # 58.99%

# Not even a 0.5% reduction
...
# M_DTI_Growth, M_DTI_Growth_1, M_DTI_Growth_12 and M_DTI_Growth_Vol_12 are significant
...
# Would need to include M_DTI_Growth as well
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_5) # 61.63% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_5) # 61.64%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_6) # 61.32% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_6) # 61.33%
...
# Exactly the same as without the volatilities
...
# M_RealIncome_Growth, M_RealIncome_Growth_1, M_RealIncome_Growth_4, M_RealIncome_Growth_12, 
# M_RealIncome_Growth_Vol_2, M_RealIncome_Growth_Vol_10 and M_RealIncome_Growth_Vol_11 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_11) # 60.79% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_11) # 60.80%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_12) # 59.70% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_12) # 59.72%

# Not even a 1% reduction

# Model with all the significant MVs
inputs_macro <- SICR_target_event ~ M_Repo_Rate + M_Repo_Rate_2 + M_Repo_Rate_3 + M_Repo_Rate_4 + M_Repo_Rate_6 + M_Repo_Rate_8 +
                                    M_Repo_Rate_10 + M_Inflation_Growth + M_Inflation_Growth_1 + M_Inflation_Growth_10 +
                                    M_DTI_Growth + M_DTI_Growth_11 + M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_8 +
...
inputs_chosen <- SICR_target_event ~ Term + BalanceLog + LN_TPE + BalanceToTerm + InterestRate_Margin +
                                     ...
                                     M_Repo_Rate + M_Inflation_Growth + M_DTI_Growth + M_DTI_Growth_11 + 
                                     M_Emp_Growth + M_RealGDP_Growth + M_RealGDP_Growth_1 + M_RealIncome_Growth
                                     # Age_Adj + AgeToTerm + M_Repo_Rate_6 + M_Inflation_Growth_1 + M_Inflation_Growth_10 +
                                     # M_DTI_Growth_12 + M_Emp_Growth_8 + M_Emp_Growth_9 + M_Emp_Growth_11 + M_Emp_Growth_12 +
                                     # M_RealGDP_Growth_11 + M_RealGDP_Growth_12 + M_RealIncome_Growth_11 + M_RealIncome_Growth_12
                                     # M_Repo_Rate_2 + M_Repo_Rate_3 + M_Repo_Rate_4 + M_Repo_Rate_10 + Receipt_InfLog + M_Repo_Rate_8
...
# Term, BalanceLog, LN_TPE, BalanceToTerm, InterestRate_Margin, TimeInPerfSpell, PerfSpell_Num, g0_Delinq, slc_acct_arr_dir_3, slc_pmnt_method,
# M_Repo_Rate, M_Repo_Rate_2, M_Repo_Rate_3,  M_Repo_Rate_8, M_Repo_Rate_10, M_Inflation_Growth, M_DTI_Growth, M_DTI_Growth_11,
# M_Emp_Growth, M_RealGDP_Growth, M_RealGDP_Growth_1, M_RealIncome_Growth, and the two value indicator variables
# The following variables have p-values greater than 5% but less than 20%:
# Receipt_InfLog, M_Repo_Rate_4
# Iteration 2
# The following variables are significant in no particular order:
# All except those listed below
# The following variables have p-values greater than 5% but less than 20%:
# Receipt_InfLog, M_Repo_Rate_4
# Too many repo rate variables - choose the two most significant variables: M_Repo_Rate and M_Repo_Rate_8. Remove the rest
# Iteration 3
# The following variables are significant in no particular order:
# All except M_Repo_Rate_8 and Receipt_InfLog - remove
# Iteration 4
# All the variables are significant

# - Compute the AUC
dat_SICR_def_train_s[, Prob_chosen_1b_iii := predict(logit_model_chosen, newdata = dat_SICR_def_train_s, type="response")] 
dat_SICR_def_valid_s[, Prob_chosen_1b_iii := predict(logit_model_chosen, newdata = dat_SICR_def_valid_s, type="response")]

auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_chosen_1b_iii) 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_chosen_1b_iii) 

# Report the AUC for the validation dataset
# Iteration 1's AUC: 87.42% | Iteration 2's AUC: 87.44% | Iteration 3's AUC: 87.45% | Iteration 4's AUC: 87.44% 

# - Analyse possible multicollinearity
inputs_all_mc1 <- SICR_target_event ~ Term + BalanceLog + BalanceToTerm + InterestRate_Margin +
...
# Real GDP growth and real income growth have VIF > 10 but this is to be expected
...
table(datSICR$SICR_target) %>% prop.table() # 95.30% Non-SICR vs 4.70% SICR
...
summary(logit_model_chosen)
# Most variables are statistically significant, except: TimeInPerfSpell, Inflation_Growth, pmnt_method_grp (but only its "MISSING DATA" bin)
# [Ad hoc] PD ratio is not statistically significant (p-value of 0.28505 and standard error of 0.000827119)

# - Score data using fitted model
datSICR_train[, Prob_chosen_1b_iii := predict(logit_model_chosen, newdata = datSICR_train, type="response")] 
datSICR_valid[, Prob_chosen_1b_iii := predict(logit_model_chosen, newdata = datSICR_valid, type="response")]
datSICR_smp[, ExpProb := predict(logit_model_chosen, newdata = datSICR_smp, type="response")]

# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_1b_iii) # 87.69% vs 87.70%
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_1b_iii) # 87.98% vs 87.96%
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 87.78% (before and after)
...
ggplot( data=datSICR_valid, aes(x=Prob_chosen_1b_iii)) + theme_bw() + 
...
datSICR_train[, Pred_chosen_1b_iii := ifelse(Prob_chosen_1b_iii >= logistic_cutoff, 1, 0)]
datSICR_valid[, Pred_chosen_1b_iii := ifelse(Prob_chosen_1b_iii >= logistic_cutoff, 1, 0)]
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_1b_iii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_1b_iii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
...
datSICR_valid[, SICR_predict_variance := sd(Prob_chosen_1b_iii), by=list(LoanID)]
standard_deviation <- round(mean(datSICR_valid$SICR_predict_variance, na.rm=T)*100, digits=1)

# - Confusion matrix
conf_mat <- datSICR_valid[, list(TN=sum(ifelse(SICR_target == 0 & Pred_chosen_1b_iii == 0, 1, 0)),
                                 FP=sum(ifelse(SICR_target == 0 & Pred_chosen_1b_iii == 1, 1, 0)),
                                 TP=sum(ifelse(SICR_target == 1 & Pred_chosen_1b_iii == 1, 1, 0)),
                                 FN=sum(ifelse(SICR_target == 1 & Pred_chosen_1b_iii == 0, 1, 0)))]
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.08587387
...
performance_measures_1b_iii <- data.frame(SICR_definition = paste0(SICR_label, "_logit"),
                                          d=p.d, s=p.s, k=p.k,
...
pack.ffdf(paste0(genObjPath, "performance_measures_", SICR_label), performance_measures_1b_iii); gc()
