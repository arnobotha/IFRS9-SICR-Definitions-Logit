# ============================== SICR-MODELLIING ===============================
# SICR-model for definition 2c(ii) using Logistic Regression (LR)
# ------------------------------------------------------------------------------
...
ggplot(plot.data_SICR_rates, aes(x=Date, y=Proportion, colour=Prop)) + 
  ...
  labs(y="SICR-incidence rates for definition with parameters k=6, d=2, and s=3", x= "Time") + 
...
# Age_Adj, Balance, FurtherLoan_Ind, FurtherLoan_Amt, AgeToTerm, BalanceToTerm, LN_TPE
...
# Age_Adj, Term, AgeToTerm and LN_TPE are insignificant

# - Remove the insignificant variables from the "raw" variable theme and analyse again
inputs_account3 <- SICR_target_event ~ Term + Receipt_Inf + Redraw_Ind + Redrawn_Amt + InterestRate_Margin 
...
inputs_account4 <- SICR_target_event ~ Receipt_InfLog + BalanceLog + BalanceToTerm + InterestRate_Margin
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_3) # 61.63%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_3) # 60.47%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_4) # 66.51%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_4) # 66.69%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_1) # 92.98%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_1) # 92.51%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_2) # 92.98%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_2) # 92.50%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_3) # 74.28% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_3) # 74.04%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_4) # 74.28% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_4) # 74.04%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_1) # 59.38% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_1) # 60.62%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_2) # 59.35% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_2) # 60.47%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_3) # 60.53% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_3) # 63.18%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_4) # 60.65% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_4) # 63.17%

# No reduction in the AUC
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_5) # 60.99% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_5) # 63.77%
...
# M_DTI_Growth_12 is the most significant (11.50% p-value)
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_6) # 61.06% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_6) # 63.92%

# No reduction in AUC, in fact, a slight increase
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_7) # 60.22% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_7) # 62.93%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_8) # 60.23% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_8) # 62.92%

# Almost no reduction in the AUC
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_9) # 61.01% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_9) # 63.79%
...
# M_RealGDP_Growth_12 is the most significant (p-value of 11.50%)
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_10) # 60.30% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_10) # 63.19%

# Slight reduction in the AUC
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_11) # 61.75% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_11) # 63.88%
...
# M_RealIncome_Growth_12 is the most significant (p-value of 14.80%)
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_12) # 61.02% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_12) # 63.70%

# Slight reduction in the AUC

# Model with all the significant MVs
inputs_macro7 <- SICR_target_event ~ M_Repo_Rate + M_Inflation_Growth + M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth + 
...
inputs_chosen <- SICR_target_event ~ Receipt_InfLog + BalanceLog + InterestRate_Margin + g0_Delinq +  slc_acct_arr_dir_3 + 
                                     slc_acct_roll_ever_24_imputed + slc_pmnt_method + slc_acct_pre_lim_perc_imputed + 
                                     M_Repo_Rate + M_Inflation_Growth + M_RealIncome_Growth + 
                                     M_RealIncome_Growth_12 + M_RealGDP_Growth + M_RealGDP_Growth_12
                                     # BalanceToTerm, TimeInPerfSpell, PerfSpell_Num, PrevDefaults, M_DTI_Growth, M_DTI_Growth_12
...
# BalanceToTerm, TimeInPerfSpell, PerfSpell_Num, PrevDefaults
# Only remove the insignificant MVs in iteration 2
# Iteration 2
# The insignificant variables are:
# Inflation growth rate, DTI growth - remove only DTI growth

# - Compute the AUC
dat_SICR_def_train_s[, Prob_chosen_2c_ii := predict(logit_model_chosen, newdata = dat_SICR_def_train_s, type="response")] 
dat_SICR_def_valid_s[, Prob_chosen_2c_ii := predict(logit_model_chosen, newdata = dat_SICR_def_valid_s, type="response")]

auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_chosen_2c_ii) 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_chosen_2c_ii) 

# Report the AUC for the validation dataset
# Iteration 1's AUC: 92.94% | Iteration 2's AUC: 93.00% 

# - Include only the numerical variables to analyse possible multicollinearity
inputs_all_mc1 <- SICR_target_event ~ Receipt_InfLog + BalanceLog + InterestRate_Margin + g0_Delinq + slc_acct_roll_ever_24_imputed + 
                                      slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Inflation_Growth + M_RealIncome_Growth + 
                                      M_RealIncome_Growth_12 + M_RealGDP_Growth + M_RealGDP_Growth_12
...
# After the initial feature selection across the 1million observations sample across 2c(i)-(iv), the following variables have been identified as significant
# across the definitions:
# InterestRate_Margin, BalanceLog, slc_pmnt_method, slc_acct_pre_lim_perc_imputed, g0_Delinq, slc_acct_arr_dir_3, slc_acct_roll_ever_24_imputed,
# M_Repo_Rate, M_Inflation_Growth, M_Emp_Growth, M_Emp_Growth_12, M_RealIncome_Growth and M_RealIncome_Growth_12

# Now, the input space should be finalized across definitions 2c(i)-(iv)
...
# Investigate the possible macroeconomic variables
# Keep repo and inflation growth
# Test employment growth
inputs_macro_all <- SICR_target_event ~ InterestRate_Margin + BalanceLog + slc_pmnt_method + slc_acct_pre_lim_perc_imputed + 
                                        g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed +
                                        M_Repo_Rate + M_Inflation_Growth + M_Emp_Growth + M_Emp_Growth_12
logit_macro_all <- glm(inputs_macro_all, data=dat_SICR_def_train_s, family="binomial")
summary(logit_macro_all)
# The 0-month and 12-month lag are significant
dat_SICR_def_valid_s[, Prob_macro_all := predict(logit_macro_all, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all) # 93.06%

# Real income growth - 0-month lag
inputs_macro_all2 <- SICR_target_event ~ InterestRate_Margin + BalanceLog + slc_pmnt_method + slc_acct_pre_lim_perc_imputed + 
                                         g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed +
                                         M_Repo_Rate + M_Inflation_Growth + M_Emp_Growth + M_Emp_Growth_12 + M_RealIncome_Growth
...
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all2) # 93.04%
# The real income growth is not significant

# Real income growth - 12-month lag
inputs_macro_all3 <- SICR_target_event ~ InterestRate_Margin + BalanceLog + slc_pmnt_method + slc_acct_pre_lim_perc_imputed + 
                                         g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed +
                                         M_Repo_Rate + M_Inflation_Growth + M_Emp_Growth + M_Emp_Growth_12 + M_RealIncome_Growth + M_RealIncome_Growth_12
...
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all3) # 93.07%
# Both are insignificant - remove
...
# Keep repo and inflation growth
# Test employment growth
inputs_macro_all <- SICR_target ~ InterestRate_Margin + BalanceLog + slc_pmnt_method + slc_acct_pre_lim_perc_imputed + 
                                  g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed +
                                  M_Repo_Rate + M_Inflation_Growth + M_Emp_Growth + M_Emp_Growth_12
logit_macro_all <- glm(inputs_macro_all, data=datSICR_train, family="binomial")
summary(logit_macro_all)
# M_Emp_Growth is significant and the 12-month lag is still quite significant (p-value of 20.37%)
datSICR_valid[, Prob_macro_all := predict(logit_macro_all, newdata = datSICR_valid, type="response")]
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all) # 82.64%

# Real income growth - 0-month lag
inputs_macro_all2 <- SICR_target ~ InterestRate_Margin + BalanceLog + slc_pmnt_method + slc_acct_pre_lim_perc_imputed + 
                                   g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed +
                                   M_Repo_Rate + M_Inflation_Growth + M_Emp_Growth + M_Emp_Growth_12 + M_RealIncome_Growth
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all2) # 81.42%
# Real income growth is rather significant, but does not meet the 25% p-value threshold

# M_RealIncome_Growth_12
inputs_macro_all3 <- SICR_target ~  InterestRate_Margin + BalanceLog + slc_pmnt_method + slc_acct_pre_lim_perc_imputed + 
                                    g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed +
                                    M_Repo_Rate + M_Inflation_Growth + M_Emp_Growth + M_Emp_Growth_12 + M_RealIncome_Growth + M_RealIncome_Growth_12
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all3) # 81.72%
# The 12-month lag is significant. Therefore, keep.
...
# - Check the event rate of each class | RECORD-LEVEL
table(datSICR$SICR_target) %>% prop.table() # 99.99% Non-SICR vs 0.10% SICR
...
# Not all variables are statistically significant, including: Interest rate margin, missing and statement levels of payment method,
# prepaid funds, arrears direction, real income growth, inflation growth, employment growth, 12-month employment growth lag
# After including PD ratio, the statistically insignificant variables are: Interest rate margin, missing and statement levels of payment method,
# prepaid funds, arrears direction, real income growth, inflation growth, employment growth, 12-month employment growth lag
# Therefore, the inclusion of PD ratio did not change the significance of any variables
# [Ad hoc] PD ratio is not statistically significant (p-value of 0.99827 and standard error of 0.000003574163)
...
datSICR_train[, Prob_chosen_2c_ii := predict(logit_model_chosen, newdata = datSICR_train, type="response")] 
datSICR_valid[, Prob_chosen_2c_ii := predict(logit_model_chosen, newdata = datSICR_valid, type="response")]
datSICR_smp[, ExpProb := predict(logit_model_chosen, newdata = datSICR_smp, type="response")]

# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_2c_ii) # 98.28%
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_2c_ii) # 81.62%
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 85.60%
...
ggplot( data=datSICR_valid, aes(x=Prob_chosen_2c_ii)) + theme_bw() + 
...
datSICR_train[, Pred_chosen_2c_ii := ifelse(Prob_chosen_2c_ii >= logistic_cutoff, 1, 0)]
datSICR_valid[, Pred_chosen_2c_ii := ifelse(Prob_chosen_2c_ii >= logistic_cutoff, 1, 0)]
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_2c_ii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_2c_ii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
...
datSICR_valid[, SICR_predict_variance := sd(Prob_chosen_2c_ii), by=list(LoanID)]
standard_deviation <- round(mean(datSICR_valid$SICR_predict_variance, na.rm=T)*100, digits=1)

# - Confusion matrix
conf_mat <- datSICR_valid[, list(TN=sum(ifelse(SICR_target == 0 & Pred_chosen_2c_ii == 0, 1, 0)),
                                 FP=sum(ifelse(SICR_target == 0 & Pred_chosen_2c_ii == 1, 1, 0)),
                                 TP=sum(ifelse(SICR_target == 1 & Pred_chosen_2c_ii == 1, 1, 0)),
                                 FN=sum(ifelse(SICR_target == 1 & Pred_chosen_2c_ii == 0, 1, 0)))]
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.628256

# A few things of concern:
# 1) Volatility in event rates due to relatively low sampling volumes in validation set
# 2) 0-counts over 20006-2007 periods using discretised output [Pred_chosen_2c_ii]
...
(g <- ggplot(port.aggr, aes(x=Date, y=EventRate, group=Type)) + theme_minimal() + 
    ...
    #annotations
    annotate(geom="text", x=as.Date("2015-12-31"), y=port.aggr[Date >= "2012-12-31" & Type=="a_Actual", mean(EventRate)]*2.7,
...
performance_measures_2c_ii <- data.frame(SICR_definition = paste0(SICR_label, "_logit"),
                                         d=p.d, s=p.s, k=p.k,
...
pack.ffdf(paste0(genObjPath, "performance_measures_", SICR_label), performance_measures_2c_ii); gc()
