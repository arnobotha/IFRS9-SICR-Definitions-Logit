...
# Not all of the variables are statistically significant
# PD ratio is not statistically significant (p-value of 0.342262 and standard error of 0.0005338)

# - Score data using fitted model
datSICR_train[, Prob_chosen_1a_v := predict(logit_model_chosen, newdata = datSICR_train, type="response")] 
datSICR_valid[, Prob_chosen_1a_v := predict(logit_model_chosen, newdata = datSICR_valid, type="response")]
datSICR_smp[, ExpProb := predict(logit_model_chosen, newdata = datSICR_smp, type="response")]

# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_1a_v) # 82.28% vs 82.29%
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_1a_v) # 82.24% (before and after)
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 82.27% (before and after)
...
ggplot( data=datSICR_valid, aes(x=Prob_chosen_1a_v)) + theme_bw() + 
...
datSICR_train[, Pred_chosen_1a_v := ifelse(Prob_chosen_1a_v >= logistic_cutoff, 1, 0)]
datSICR_valid[, Pred_chosen_1a_v := ifelse(Prob_chosen_1a_v >= logistic_cutoff, 1, 0)]
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_1a_v, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_1a_v, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
...
datSICR_valid[, SICR_predict_variance := sd(Prob_chosen_1a_v), by=list(LoanID)]
standard_deviation <- round(mean(datSICR_valid$SICR_predict_variance, na.rm=T)*100, digits=1)

# - Confusion matrix
conf_mat <- datSICR_valid[, list(TN=sum(ifelse(SICR_target == 0 & Pred_chosen_1a_v == 0, 1, 0)),
                                 FP=sum(ifelse(SICR_target == 0 & Pred_chosen_1a_v == 1, 1, 0)),
                                 TP=sum(ifelse(SICR_target == 1 & Pred_chosen_1a_v == 1, 1, 0)),
                                 FN=sum(ifelse(SICR_target == 1 & Pred_chosen_1a_v == 0, 1, 0)))]
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.1154424
...
pack.ffdf(paste0(genPath, "performance_measures_", SICR_label), performance_measures_1a_v); gc()
