# ============================== SICR-MODELLIING ===============================
# SICR-model for definition 1b(iv) using Logistic Regression (LR)
# ------------------------------------------------------------------------------
...
ggplot(plot.data_SICR_rates, aes(x=Date, y=Proportion, colour=Prop)) + 
  ...
  labs(y="SICR-incidence rates for definition with parameters k=12, d=1, and s=2", x= "Time") + 
...
# Age_Adj, FurtherLoan_Amt (so we need to remove FurtherLoan_Ind as well) and AgeToTerm
...
inputs_account3 <- SICR_target_event ~ Term + Receipt_Inf + Balance + Redraw_Ind + Redrawn_Amt + InterestRate_Margin + LN_TPE + BalanceToTerm 
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_2) # 64.19%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_2) # 63.83%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_3) # 60.61%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_3) # 60.07%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_1) # 82.51%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_1) # 82.30%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_2) # 82.52%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_2) # 82.31%

# Slightly higher AUC on the training dataset using the value indicator variables.
# Slightly lower p-value for PerfSpell_Num and TimeInPerfSpell using the value indicator.
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_3) # 74.17% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_3) # 74.09%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_4) # 74.17% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_4) # 74.09%
...
# M_Repo_Rate, M_Repo_Rate_2, M_Repo_Rate_4, M_Repo_Rate_6, M_Repo_Rate_8, M_Repo_Rate_10, M_Repo_Rate_11, M_Repo_Rate_12,
# M_Repo_Rate_Vol_3, M_Repo_Rate_Vol_4, M_Repo_Rate_Vol_7, M_Repo_Rate_Vol_10 and M_Repo_Rate_Vol_12 are significant
...
# M_Repo_Rate, M_Repo_Rate_2, M_Repo_Rate_4, M_Repo_Rate_6, M_Repo_Rate_8, M_Repo_Rate_10 and M_Repo_Rate_12 are significant

# - Compute the AUC to investigate the difference in he AUC of using the volatility variables
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_1) # 59.70% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_1) # 59.72%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_2) # 58.14% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_2) # 58.15%

# More than a 1% reduction in the AUC, but not significant enough to warrant the inclusion of the volatility variables
...
# M_Inflation_Growth_Vol_2, M_Inflation_Growth_Vol_4, M_Inflation_Growth_Vol_5, M_Inflation_Growth_Vol_7, M_Inflation_Growth_Vol_8,
...
# M_Inflation_Growth, M_Inflation_Growth_1, M_Inflation_Growth_3, M_Inflation_Growth_10, M_Inflation_Growth_11 and M_Inflation_Growth_12 
# are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_3) # 58.33% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_3) # 58.33%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_4) # 57.84% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_4) # 57.84%

# Around a 0.5% reduction in the AUC
...
# M_DTI_Growth_12, M_DTI_Growth_Vol_2 and M_DTI_Growth_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_5) # 61.27% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_5) # 61.28%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_6) # 60.86% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_6) # 60.88%

# Very small reduction in the AUC, around 0.4%
...
# M_Emp_Growth, M_Emp_Growth_1, M_Emp_Growth_8, M_Emp_Growth_9, M_Emp_Growth_11, M_Emp_Growth_12 and M_Emp_Growth_Vol_12 are significant
...
# M_Emp_Growth, M_Emp_Growth_1, M_Emp_Growth_8, M_Emp_Growth_9, M_Emp_Growth_11, M_Emp_Growth_12 are significant

# - Compute the AUC to investigate the difference in the AUC of using the volatility variables
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_7) # 59.88% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_7) # 59.89%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_8) # 58.86% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_8) # 58.85%

# Less than a 1% reduction in AUC
...
# M_RealGDP_Growth, M_RealGDP_Growth_1 and M_RealGDP_Growth_12 are significant
...
# M_RealGDP_Growth, M_RealGDP_Growth_1, M_RealGDP_Growth_11 and M_RealGDP_Growth_12 are significant

# No volatility variables are significant
...
# M_RealIncome_Growth, M_RealIncome_Growth_11, M_RealIncome_Growth_12, M_RealIncome_Growth_Vol_2, M_RealIncome_Growth_Vol_10,
# M_RealIncome_Growth_Vol_11 are significant
...
inputs_macro <- SICR_target_event ~ M_Repo_Rate + M_Repo_Rate_2 + M_Repo_Rate_4 + M_Repo_Rate_6 + M_Repo_Rate_8 + M_Repo_Rate_10 + 
                                    M_Repo_Rate_12 + M_Inflation_Growth + M_Inflation_Growth_1 + M_Inflation_Growth_3 +
                                    M_Inflation_Growth_10 + M_Inflation_Growth_11 + M_Inflation_Growth_12 + M_DTI_Growth +
                                    M_DTI_Growth_11 + M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_1 + M_Emp_Growth_8 +
                                    M_Emp_Growth_9 + M_Emp_Growth_11 + M_Emp_Growth_12 + M_RealGDP_Growth + M_RealGDP_Growth_1 +
                                    M_RealGDP_Growth_11 + M_RealGDP_Growth_12 + M_RealIncome_Growth + M_RealIncome_Growth_11 +
                                    M_RealIncome_Growth_12
...
inputs_chosen <- SICR_target_event ~ Term + Receipt_InfLog + BalanceLog + LN_TPE + BalanceToTerm + InterestRate_Margin +
                                     TimeInPerfSpell + PerfSpell_Num + g0_Delinq + slc_acct_arr_dir_3 + 
                                     slc_acct_roll_ever_24_imputed:value_ind_slc_acct_roll_ever_24 + 
                                     slc_pmnt_method + slc_acct_pre_lim_perc_imputed:value_ind_slc_acct_pre_lim_perc +
                                     M_Repo_Rate + M_Repo_Rate_10 + M_DTI_Growth + M_DTI_Growth_11 +
                                     M_Emp_Growth + M_RealGDP_Growth + M_RealGDP_Growth_12 + M_RealIncome_Growth
                                     # AgeToTerm + PrevDefaults + M_Repo_Rate_6 + M_Repo_Rate_12 + M_Inflation_Growth_3 + M_Inflation_Growth_11 +
                                     # M_DTI_Growth_12 + M_Emp_Growth_11 + M_RealGDP_Growth_1 + M_RealIncome_Growth_11 + M_RealIncome_Growth_12 +
                                     # Age_Adj + M_Repo_Rate_8 + M_Inflation_Growth_1 + M_Repo_Rate_2 + M_Repo_Rate_4 + M_Inflation_Growth_10 +
                                     # M_Emp_Growth_1, M_Emp_Growth_8, M_Emp_Growth_12 + M_RealGDP_Growth_11 + M_Inflation_Growth + 
                                     # M_Inflation_Growth_12 + M_Emp_Growth_9
                                      
logit_model_chosen <- glm(inputs_chosen, data=dat_SICR_def_train_s, family="binomial")
summary(logit_model_chosen)

# NOTE: Significance of the variables
# Iteration 1
# The following variables are significant in no particular order:
# Term, Receipt_InfLog, BalanceLog, LN_TPE, BalanceToTerm, InterestRate_Margin, TimeInPerfSpell, PerfSpell_Num, g0_Delinq, slc_acct_arr_dir_3,
# slc_pmnt_method, M_Repo_Rate, M_Repo_Rate_2, M_Repo_Rate_10, M_DTI_Growth, and the two value indicator variables 
# The following variable have p-values greater than 5% but less than 20%:
# Age_Adj, M_Repo_Rate_4, M_Repo_Rate_8, M_Inflation_Growth_1, M_Inflation_Growth_10, M_Inflation_Growth_12, M_DTI_Growth_11,
# M_Emp_Growth_1, M_Emp_Growth_8, M_Emp_Growth_9, M_Emp_Growth_12, M_RealGDP_Growth_11, M_RealGDP_Growth_12
# Iteration 2
# The following variables are significant in no particular order:
# Term, Receipt_InfLog, BalanceLog, LN_TPE, BalanceToTerm, InterestRate_Margin, TimeInPerfSpell, PerfSpell_Num, g0_Delinq, slc_acct_arr_dir_3,
# slc_pmnt_method, M_Repo_Rate, M_Repo_Rate_2, M_Repo_Rate_10, M_Inflation_Growth_12, M_DTI_Growth, M_DTI_Growth_11, M_Emp_Growth_8,
# M_Emp_Growth_9, M_RealGDP_Growth_11, M_RealGDP_Growth_12 and the value indicator variables
# The following variable have p-values greater than 5% but less than 20%:
# M_Repo_Rate_4, M_Inflation_Growth_10, M_Emp_Growth_1, M_Emp_Growth_12
# Iteration 3
# The following variables are significant in no particular order:
# Choose the two most significant MVs: 
# Repo: 0 and 10
# Inflation growth: 0 and 12
# Employment growth: 0 and 9
# Real GDP growth: 0 and 12
# Iteration 4
# The following variables are not significant:
# Inflation growth variables, M_Emp_Growth_9, M_RealGDP_Growth (but need this for the lagged version that is in fact significant)
# Iteration 5
# M_RealGDP_Growth (but need this for the lagged version that is in fact significant)

# - Compute the AUC
dat_SICR_def_train_s[, Prob_chosen_1b_iv := predict(logit_model_chosen, newdata = dat_SICR_def_train_s, type="response")] 
dat_SICR_def_valid_s[, Prob_chosen_1b_iv := predict(logit_model_chosen, newdata = dat_SICR_def_valid_s, type="response")]

auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_chosen_1b_iv) 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_chosen_1b_iv) 

# Iteration 1's AUC: 85.87% | Iteration 2's AUC: 85.87% | Iteration 3's AUC: 85.87% | Iteration 4's AUC: 85.86% | Iteration 5's AUC: 85.87%
...
inputs_all_mc1 <- SICR_target_event ~ Term + Receipt_InfLog + BalanceLog + BalanceToTerm + InterestRate_Margin +
                                      TimeInPerfSpell + PerfSpell_Num + g0_Delinq + 
                                      slc_acct_roll_ever_24_imputed:value_ind_slc_acct_roll_ever_24 + 
...
# The macroeconomic variables have VIF > 10, but this is expected
...
table(datSICR$SICR_target) %>% prop.table() # 95.36% Non-SICR vs 4.64% SICR
...
summary(logit_model_chosen)
# Most variables are statistically significant, except: TimeInPerfSpell, M_Inflation_Growth, pmnt_method_grp (but only its "Salary/Suspense" bin)
# [Ad hoc] PD ratio is not statistically significant (p-value of 0.68614 and standard error of 0.0000009190) as well as inflation growth

# - Score data using fitted model
datSICR_train[, Prob_chosen_1b_iv := predict(logit_model_chosen, newdata = datSICR_train, type="response")] 
datSICR_valid[, Prob_chosen_1b_iv := predict(logit_model_chosen, newdata = datSICR_valid, type="response")]
datSICR_smp[, ExpProb := predict(logit_model_chosen, newdata = datSICR_smp, type="response")]

# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_1b_iv) # 86.15% (before and after)
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_1b_iv) # 86.45% (before and after)
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 86.24% (before and after)
...
ggplot( data=datSICR_valid, aes(x=Prob_chosen_1b_iv)) + theme_bw() + 
...
datSICR_train[, Pred_chosen_1b_iv := ifelse(Prob_chosen_1b_iv >= logistic_cutoff, 1, 0)]
datSICR_valid[, Pred_chosen_1b_iv := ifelse(Prob_chosen_1b_iv >= logistic_cutoff, 1, 0)]
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_1b_iv, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_1b_iv, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
...
datSICR_valid[, SICR_predict_variance := sd(Prob_chosen_1b_iv), by=list(LoanID)]
standard_deviation <- round(mean(datSICR_valid$SICR_predict_variance, na.rm=T)*100, digits=1)

# - Confusion matrix
conf_mat <- datSICR_valid[, list(TN=sum(ifelse(SICR_target == 0 & Pred_chosen_1b_iv == 0, 1, 0)),
                                 FP=sum(ifelse(SICR_target == 0 & Pred_chosen_1b_iv == 1, 1, 0)),
                                 TP=sum(ifelse(SICR_target == 1 & Pred_chosen_1b_iv == 1, 1, 0)),
                                 FN=sum(ifelse(SICR_target == 1 & Pred_chosen_1b_iv == 0, 1, 0)))]
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.1030042 
...
performance_measures_1b_iv <- data.frame(SICR_definition = paste0(SICR_label, "_logit"),
                                         d=p.d, s=p.s, k=p.k,
...
pack.ffdf(paste0(genObjPath, "performance_measures_", SICR_label), performance_measures_1b_iv); gc()
