...
# PD ratio is not statistically significant (p-value of 0.251041 and standard error of 0.0002814)

# NOTES: On the significance of the variables
# The only insignificant variable is Term (p-value of 76.98%)
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_1c_ii, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_1c_ii, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
...
pack.ffdf(paste0(genPath, "performance_measures_", SICR_label), performance_measures_1c_ii); gc()
