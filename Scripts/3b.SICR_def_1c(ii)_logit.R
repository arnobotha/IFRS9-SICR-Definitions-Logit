# ============================== SICR-MODELLIING ===============================
# SICR-model for definition 1c(ii) using Logistic Regression (LR)
# ------------------------------------------------------------------------------
...
ggplot(plot.data_SICR_rates, aes(x=Date, y=Proportion, colour=Prop)) + 
  ...
  labs(y="SICR-incidence rates for definition with parameters k=6, d=1, and s=3", x= "Time") + 
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_2) # 66.04%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_2) # 66.17%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_3) # 61.27%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_3) # 61.16%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_1) # 89.39%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_1) # 89.87%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_2) # 89.37%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_2) # 89.85%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_3) # 75.91% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_3) # 76.04%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_4) # 75.91% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_4) # 76.04%
...
# M_Repo_Rate, M_Repo_Rate_2, M_Repo_Rate_4, M_Repo_Rate_6, M_Repo_Rate_Vol_3, M_Repo_Rate_Vol_7, M_Repo_Rate_Vol_10 and M_Repo_Rate_Vol_12 are significant
...
# M_Repo_Rate, M_Repo_Rate_2, M_Repo_Rate_6 and M_Repo_Rate_10 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_1) # 61.06% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_1) # 61.09%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_2) # 60.55% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_2) # 60.58%
...
# M_Inflation_Growth_Vol_2 and M_Inflation_Growth_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_3) # 60.93% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_3) # 60.95%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_4) # 60.61% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_4) # 60.62%
...
# M_DTI_Growth, M_DTI_Growth_11 and M_DTI_Growth_12 are significant
...
# No volatility-related variables were significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_7) # 61.19% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_7) # 61.20%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_8) # 59.78% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_8) # 59.78%

# RESULT: Higher reduction than the rest of the variables
...
# M_RealGDP_Growth, M_RealGDP_Growth_12 and M_RealGDP_Growth_Vol_2 are significant
...
# M_RealGDP_Growth_11 and M_RealGDP_Growth_12 are significant
# Would need to include the unlagged version as well M_RealGDP_Growth
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_9) # 62.35% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_9) # 62.36%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_10) # 61.43% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_10) # 61.44%

# RESULT: Less than a 1% reduction in AUC
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_11) # 62.09% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_11) # 62.10%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_12) # 60.32% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_12) # 60.32%
...
inputs_macro <- SICR_target_event ~ M_Repo_Rate + M_Repo_Rate_2 + M_Repo_Rate_6 + M_Repo_Rate_10 + M_Inflation_Growth + M_Inflation_Growth_1 + M_Inflation_Growth_10 +
                                    M_DTI_Growth + M_DTI_Growth_11 + M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_8 + M_Emp_Growth_9 + M_Emp_Growth_11 + M_Emp_Growth_12 +
...
inputs_chosen <- SICR_target_event ~ Age_Adj + Receipt_InfLog + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num + g0_Delinq + slc_acct_arr_dir_3 +
                                     slc_acct_roll_ever_24_imputed + slc_pmnt_method + slc_acct_pre_lim_perc_imputed + M_Repo_Rate +  + M_Repo_Rate_6 +
                                     M_Inflation_Growth + M_Inflation_Growth_1 + M_DTI_Growth + M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_12 +
                                     M_RealGDP_Growth +  + M_RealIncome_Growth
                                     # AgeToTerm + BalanceToTerm + M_Repo_Rate_2 + M_Inflation_Growth_10 + M_DTI_Growth_11 + M_Emp_Growth_8 + M_Emp_Growth_9 + 
                                     # M_Emp_Growth_11 + M_RealIncome_Growth_11 + M_RealIncome_Growth_12 + M_RealGDP_Growth_11 + M_RealGDP_Growth_12 + M_Repo_Rate_10
                                     # M_RealIncome_Growth_8 + Term + LN_TPE + PrevDefaults + M_RealIncome_Growth_9
...
# Age_Adj, Receipt_InfLog, BalanceLog, InterestRate_Margin, TimeInPerfSpell, PerfSpell_Num, g0_Delinq, slc_acct_arr_dir_3, slc_acct_roll_ever_24_imputed, slc_pmnt_method,
# slc_acct_pre_lim_perc_imputed, M_Repo_Rate, M_Repo_Rate_6, M_Repo_Rate_10, M_Inflation_Growth_1, M_DTI_Growth, M_Emp_Growth, M_Emp_Growth_12
# M_RealGDP_Growth, M_RealGDP_Growth_11, M_RealIncome_Growth
# The following variables have a p-value greater than 5% but less than 20% (still rather significant):
# Term, LN_TPE, PrevDefaults, M_DTI_Growth_12, M_RealGDP_Growth_12, M_RealIncome_Growth_8, M_RealIncome_Growth_9 
# Iteration 2
# The following variables are not significant in no particular order:
# M_RealGDP_Growth_11, M_RealGDP_Growth_12, M_Inflation_Growth (but the lag is, so need to include)
# The following variables have a p-value greater than 5% but less than 20% (still rather significant):
# Term, LN_TPE, PrevDefaults
# Remove M_Repo_Rate_10 since the other repo rate variable is more important
# Iteration 3
# Remove M_RealIncome_Growth_8 because lag 9 is more significant
# The following variables have a p-value greater than 5% but less than 20% (still rather significant):
# Term, LN_TPE, PrevDefaults
# Iteration 4
# Remove Term, LN_TPE, PrevDefaults and M_RealIncome_Growth_9
# Iteration 5
# The suspense group is insigificant and the unlagged version of inflation growth

# - Compute the AUC
dat_SICR_def_train_s[, Prob_chosen_1c_ii := predict(logit_model_chosen, newdata = dat_SICR_def_train_s, type="response")] 
dat_SICR_def_valid_s[, Prob_chosen_1c_ii := predict(logit_model_chosen, newdata = dat_SICR_def_valid_s, type="response")]

auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_chosen_1c_ii) 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_chosen_1c_ii)

# Report the AUC for the validation dataset
# Iteration 1's AUC: 91.41% | Iteration 2's AUC: 91.43% | Iteration 3's AUC: 91.43% | Iteration 4's AUC: 91.43% | Iteration 5's AUC: 91.43% 

# - Analyse possible multicollinearity
inputs_all_mc1 <- SICR_target_event ~ Age_Adj + Receipt_InfLog + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num + g0_Delinq +
                                      slc_acct_roll_ever_24_imputed + slc_acct_pre_lim_perc_imputed + M_Repo_Rate +  + M_Repo_Rate_6 +
                                      M_Inflation_Growth + M_Inflation_Growth_1 + M_DTI_Growth + M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_12 +
                                      M_RealGDP_Growth +  + M_RealIncome_Growth
...
# Only the macroeconomic variables have VIF > 10 that is to be expected

# - Final variable selection
# After the initial feature selection across the 1million observations sample across 1c(i)-(iv), the following variables have been identified as significant
# across the definitions:
# Age_Adj, Term, BalanceLog, InterestRate_Margin, 
...
# M_Repo_Rate, M_Repo_Rate_12, M_Inflation_Growth, M_Inflation_Growth_12, M_DTI_Growth, M_DTI_Growth_12, 
# M_RealGDP_Growth and M_RealIncome_Growth

# Investigate to either use Age_Adj or Term
# Investigate the MVs to use

# Now, the input space should be finalized across definitions 1c(i)-(iv)
...
# All the variables excluding MVs to choose between Age_Adj or Term
# Age_Adj
inputs_acc_level <- SICR_target_event ~ Age_Adj + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                        g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
...
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_acc_level) # 90.85%

# Term
inputs_acc_level2 <- SICR_target_event ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                         g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                         slc_acct_pre_lim_perc_imputed
...
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_acc_level2) # 90.86%
# keep term

# All the possible macroeconomic variables
# First, check the repo and inflation
inputs_macro_all <- SICR_target_event ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                        g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                        slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Repo_Rate_12 + M_Inflation_Growth + 
                                        M_Inflation_Growth_12
logit_macro_all <- glm(inputs_macro_all, data=dat_SICR_def_train_s, family="binomial")
summary(logit_macro_all)
# The 12-month repo and inflation growth rate lags are not significant (p-value of 5.89% and 9.35%)
dat_SICR_def_valid_s[, Prob_macro_all := predict(logit_macro_all, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all) # 91.37%

# Include DTI growth
inputs_macro_all2 <- SICR_target_event ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                         g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                         slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Repo_Rate_12 + M_Inflation_Growth + 
                                         M_Inflation_Growth_12 + M_DTI_Growth + M_DTI_Growth_12
logit_macro_all2 <- glm(inputs_macro_all2, data=dat_SICR_def_train_s, family="binomial")
summary(logit_macro_all2)
# The inflation growth variables are not significant now
dat_SICR_def_valid_s[, Prob_macro_all2 := predict(logit_macro_all2, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all2) # 91.44%

# Include real GDP or real income
# Real GDP growth
inputs_macro_all3 <- SICR_target_event ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                         g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                         slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Repo_Rate_12 + M_Inflation_Growth + 
                                         M_Inflation_Growth_12 + M_DTI_Growth + M_DTI_Growth_12 + M_RealGDP_Growth
logit_macro_all3 <- glm(inputs_macro_all3, data=dat_SICR_def_train_s, family="binomial")
summary(logit_macro_all3)
# Now, the repo rate of lag 12 months is insignificant as well as the inflation growth variables with all the other
...
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all3) # 91.43%

# Real income growth
inputs_macro_all4 <- SICR_target_event ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                         g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                         slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Repo_Rate_12 + M_Inflation_Growth + 
                                         M_Inflation_Growth_12 + M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth
logit_macro_all4 <- glm(inputs_macro_all4, data=dat_SICR_def_train_s, family="binomial")
summary(logit_macro_all4)
# Only the nflation growth variables are insignificant with all the other variables being very significant
dat_SICR_def_valid_s[, Prob_macro_all4 := predict(logit_macro_all4, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all4) # 91.44%
# choose real income growth

# Remove the 12-month inflation growth variable
inputs_macro_all5 <- SICR_target_event ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                         g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                         slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Repo_Rate_12 + M_Inflation_Growth + 
                                         M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth
logit_macro_all5 <- glm(inputs_macro_all5, data=dat_SICR_def_train_s, family="binomial")
summary(logit_macro_all5)
# Only inflation growth is insignificant, but still significant enough
dat_SICR_def_valid_s[, Prob_macro_all5 := predict(logit_macro_all5, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all5) # 91.44%

# Investigate if the 12-month repo rate is removed
inputs_macro_all6 <- SICR_target_event ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                         g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                         slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Inflation_Growth + 
                                         M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth
logit_macro_all6 <- glm(inputs_macro_all6, data=dat_SICR_def_train_s, family="binomial")
summary(logit_macro_all6)
# Inflation growth is significant
dat_SICR_def_valid_s[, Prob_macro_all6 := predict(logit_macro_all6, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all6) # 91.44%
...
table(datSICR$SICR_target) %>% prop.table() # 96.16% Non-SICR vs 3.84% SICR
...
# Age_Adj or Term
# Age_Adj
inputs_acc_level <- SICR_target ~ Age_Adj + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                  g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                  slc_acct_pre_lim_perc_imputed
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_acc_level) # 91.07%

# Term
inputs_acc_level2 <- SICR_target ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                   g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                   slc_acct_pre_lim_perc_imputed
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_acc_level2) # 91.07%
# No drop in AUC, so safe to use Term

# All the possible macroeconomic variables
# First, check the repo and inflation
inputs_macro_all <- SICR_target ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                  g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                  slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Repo_Rate_12 + M_Inflation_Growth + 
                                  M_Inflation_Growth_12
logit_macro_all <- glm(inputs_macro_all, data=datSICR_train, family="binomial")
summary(logit_macro_all)
# The repo and inflation growth rate 12-month lags are insignificant
datSICR_valid[, Prob_macro_all := predict(logit_macro_all, newdata = datSICR_valid, type="response")]
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all) # 91.47%

# Include DTI growth
inputs_macro_all2 <- SICR_target ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                   g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                   slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Repo_Rate_12 + M_Inflation_Growth + 
                                   M_Inflation_Growth_12 + M_DTI_Growth + M_DTI_Growth_12
logit_macro_all2 <- glm(inputs_macro_all2, data=datSICR_train, family="binomial")
summary(logit_macro_all2)
# The 12-month repo and inflation growth rate are significant, as well as the inflation growth rate without any lag
datSICR_valid[, Prob_macro_all2 := predict(logit_macro_all2, newdata = datSICR_valid, type="response")]
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all2) # 91.53%

# Include real GDP or real income
# Real GDP
inputs_macro_all3 <- SICR_target ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                   g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                   slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Repo_Rate_12 + M_Inflation_Growth + 
                                   M_Inflation_Growth_12 + M_DTI_Growth + M_DTI_Growth_12 + M_RealGDP_Growth
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all3) # 91.54%

# Real income
inputs_macro_all4 <- SICR_target ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                   g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                   slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Repo_Rate_12 + M_Inflation_Growth + 
                                   M_Inflation_Growth_12 + M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all4) # 91.55%
# real income is appropriate

# Remove the 12-month inflation growth variable
inputs_macro_all5 <- SICR_target ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                   g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                   slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Repo_Rate_12 + M_Inflation_Growth + 
                                   M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all5) # 91.55%

# Remove the 12-month repo rate lag
inputs_macro_all6 <- SICR_target ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                                   g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                                   slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Inflation_Growth + 
                                   M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth
logit_macro_all6 <- glm(inputs_macro_all6, data=datSICR_train, family="binomial")
summary(logit_macro_all6)
datSICR_valid[, Prob_macro_all6 := predict(logit_macro_all6, newdata = datSICR_valid, type="response")]
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all6) # 91.56%
# inflation growth is more significant, so safe to remove the 12-month repo rate lag

# - Additional investigations before finalizing the input space
# Comparison of the binned slc_pmnt_method
# original
inputs_check1 <- SICR_target ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                               g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method +
                               slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Inflation_Growth + 
                               M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_check1) # 91.56%

# binned version
inputs_check2 <- SICR_target ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                               g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + pmnt_method_grp +
                               slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Inflation_Growth + 
                               M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_check2) # 91.55%
# very small reduction in AUC, so it is fine to use the binned version

# Final variables:
# Term, BalanceLog, InterestRate_Margin, TimeInPerfSpell, PerfSpell_Num, g0_Delinq, slc_acct_arr_dir_3, 
# slc_acct_roll_ever_24_imputed, pmnt_method_grp, slc_acct_pre_lim_perc_imputed, M_Repo_Rate, M_Inflation_Growth,
# M_DTI_Growth, M_DTI_Growth_12, M_RealIncome_Growth
...
# - Define model form
inputs_chosen <- SICR_target ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num + PD_ratio +
                               g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + pmnt_method_grp +
                               slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Inflation_Growth + 
                               M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth
...
summary(logit_model_chosen)
# Most variables are statistically significant, except: Term, pmnt_method_grp (but only its "MISSING DATA" bin), M_Inflation_Growth
# [Ad hoc] PD ratio is not statistically significant (p-value of 0.251041 and standard error of 0.0002814)

# - Score data using fitted model
datSICR_train[, Prob_chosen_1c_ii := predict(logit_model_chosen, newdata = datSICR_train, type="response")] 
datSICR_valid[, Prob_chosen_1c_ii := predict(logit_model_chosen, newdata = datSICR_valid, type="response")]
datSICR_smp[, ExpProb := predict(logit_model_chosen, newdata = datSICR_smp, type="response")]

# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_1c_ii) # 91.09% (before and after)
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_1c_ii) # 91.55% (before and after)
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 91.23% (before and after)
...
ggplot( data=datSICR_valid, aes(x=Prob_chosen_1c_ii)) + theme_bw() + 
...
datSICR_train[, Pred_chosen_1c_ii := ifelse(Prob_chosen_1c_ii >= logistic_cutoff, 1, 0)]
datSICR_valid[, Pred_chosen_1c_ii := ifelse(Prob_chosen_1c_ii >= logistic_cutoff, 1, 0)]
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_1c_ii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_1c_ii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
...
datSICR_valid[, SICR_predict_variance := sd(Prob_chosen_1c_ii), by=list(LoanID)]
standard_deviation <- round(mean(datSICR_valid$SICR_predict_variance, na.rm=T)*100, digits=1)

# - Confusion matrix
conf_mat <- datSICR_valid[, list(TN=sum(ifelse(SICR_target == 0 & Pred_chosen_1c_ii == 0, 1, 0)),
                                 FP=sum(ifelse(SICR_target == 0 & Pred_chosen_1c_ii == 1, 1, 0)),
                                 TP=sum(ifelse(SICR_target == 1 & Pred_chosen_1c_ii == 1, 1, 0)),
                                 FN=sum(ifelse(SICR_target == 1 & Pred_chosen_1c_ii == 0, 1, 0)))]
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.09537613
...
(g <- ggplot(port.aggr, aes(x=Date, y=EventRate, group=Type)) + theme_minimal() + 
    ...
    annotate(geom="text", x=as.Date("2015-12-31"), y=port.aggr[Date >= "2012-12-31" & Type=="a_Actual", mean(EventRate)]*3.1,
...
performance_measures_1c_ii <- data.frame(SICR_definition = paste0(SICR_label, "_logit"),
                                         d=p.d, s=p.s, k=p.k,
...
pack.ffdf(paste0(genObjPath, "performance_measures_", SICR_label), performance_measures_1c_ii); gc()
