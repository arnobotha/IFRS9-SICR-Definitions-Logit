# ============================== SICR-MODELLIING ===============================
# SICR-model for definition 2a(i) using Logistic Regression (LR)
# ------------------------------------------------------------------------------
...
ggplot(plot.data_SICR_rates, aes(x=Date, y=Proportion, colour=Prop)) + 
  ...
  labs(y="SICR-incidence rates for definition with parameters k=3, d=2, and s=1", x= "Time") + 
...
# Term, FurtherLoan_Ind, FurtherLoan_Amt, AgeToTerm and BalanceToTerm
...
# AgeToTerm and BalanceToTerm are insignificant

# - Remove the insignificant variables from the "raw" variable theme and analyse again
inputs_account3 <- SICR_target_event ~ Age_Adj + Receipt_Inf + Balance + Redraw_Ind + Redrawn_Amt + InterestRate_Margin + LN_TPE
...
inputs_account4 <- SICR_target_event ~ Age_Adj + Term + Receipt_InfLog + BalanceLog + LN_TPE + InterestRate_Margin
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_3) # 63.61%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_3) # 64.91%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_4) # 67.73%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_4) # 69.32%
...
# TimeInPerfSpell and PerfSpell_Num are insignificant
...
# Since both of these variables are significant in the other definitions, keep until all the variables are investigated together
...
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_1) # 88.56%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_2) # 88.95%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_2) # 88.56%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_3) # 76.15% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_3) # 75.52%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_4) # 76.15% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_4) # 75.52%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_1) # 57.15% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_1) # 57.15%
...
# M_Repo_Rate is the most significant at 8.70%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_2) # 57.15% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_2) # 57.15%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_3) # 58.52% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_3) # 58.58%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_4) # 58.17% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_4) # 58.20%
...
# M_DTI_Growth_12 is the most significant at 6.35%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_5) # 59.50% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_5) # 59.54%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_6) # 59.44% 
...
# M_Emp_Growth_12 and M_Emp_Growth_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_7) # 58.23% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_7) # 58.30%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_8) # 57.63% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_8) # 57.72%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_9) # 59.49% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_9) # 59.58%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_10) # 58.19% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_10) # 58.28%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_11) # 59.27% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_11) # 59.32%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_12) # 57.93% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_12) # 57.97%
...
inputs_chosen <- SICR_target_event ~ Term + Receipt_InfLog + BalanceLog + InterestRate_Margin + TimeInPerfSpell + g0_Delinq + PrevDefaults + 
                                     slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + slc_pmnt_method + slc_acct_pre_lim_perc_imputed + M_Repo_Rate + 
                                     M_Inflation_Growth + M_DTI_Growth + M_Emp_Growth + M_Emp_Growth_12 + M_RealGDP_Growth + M_RealGDP_Growth_12
                                     # Age_Adj + LN_TPE + PerfSpell_Num + M_RealIncome_Growth_12 + M_DTI_Growth_12 + M_RealIncome_Growth
...
# Term, Receipt_InfLog, BalanceLog, InterestRate_Margin, g0_Delinq, PrevDefaults, slc_acct_arr_dir_3, slc_acct_roll_ever_24_imputed, slc_pmnt_method,
# slc_acct_pre_lim_perc_imputed as well as most of the MVs
# Iteration 2
# The following variables are not significant:
# PerfSpell_Num and M_RealIncome_Growth_12
# Iteration 3
# Term is not as significant (p-value of 18.16%), M_DTI_Growth_12 is not significant, M_RealGDP_Growth is not significant but M_RealGDP_Growth_12
# is quite significant with a p-value of 11.26% (so need to keep the unlagged version), as well as M_RealIncome_Growth and M_Inflation_Growth are not significant
# Iteration 4
# M_Inflation_Growth and M_RealGDP_Growth are not significant
# M_RealGDP_Growth_12 and Term are semi-significant (p-values below 20%)

# - Compute the AUC
dat_SICR_def_train_s[, Prob_chosen_2a_i := predict(logit_model_chosen, newdata = dat_SICR_def_train_s, type="response")] 
dat_SICR_def_valid_s[, Prob_chosen_2a_i := predict(logit_model_chosen, newdata = dat_SICR_def_valid_s, type="response")]

auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_chosen_2a_i) 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_chosen_2a_i) 

# Report the AUC for the validation dataset
# Iteration 1's AUC: 91.82% | Iteration 2's AUC: 91.81% | Iteration 3's AUC: 91.81% | Iteration 4's AUC: 91.81%

# - Include only the numerical variables to analyse possible multicollinearity
inputs_all_mc1 <- SICR_target_event ~ Term + Receipt_InfLog + BalanceLog + InterestRate_Margin + TimeInPerfSpell + g0_Delinq + PrevDefaults + 
                                      slc_acct_roll_ever_24_imputed + slc_acct_pre_lim_perc_imputed + M_Repo_Rate + 
                                      M_Inflation_Growth + M_DTI_Growth + M_Emp_Growth + M_Emp_Growth_12 + M_RealGDP_Growth + M_RealGDP_Growth_12
...
# Not all variables are statistically significant, including: interest rate margin, time in performing spell, performing spell number,
# the missing and rolling groups of the account direction variable, repo rate, DTI growth and 12-month lagged DTI growth
# After including PD ratio, the statistically insignificant variables are: interest rate margin, time in performing spell, performing spell number,
# the missing and rolling groups of the account direction variable, repo rate, DTI growth and 12-month lagged DTI growth
# Therefore, the inclusion of PD ratio did not change the significance of any variables
# [Ad hoc] PD ratio is not statistically significant (p-value of 0.9761 and standard error of 0.00000000226235)
...
datSICR_train[, Prob_chosen_2a_i := predict(logit_model_chosen, newdata = datSICR_train, type="response")] 
datSICR_valid[, Prob_chosen_2a_i := predict(logit_model_chosen, newdata = datSICR_valid, type="response")]
datSICR_smp[, ExpProb := predict(logit_model_chosen, newdata = datSICR_smp, type="response")]

# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_2a_i) # 91.92% (before and after)
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_2a_i) # 91.81% (before and after)
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 91.88% (before and after)
...
ggplot( data=datSICR_valid, aes(x=Prob_chosen_2a_i)) + theme_bw() + 
...
datSICR_train[, Pred_chosen_2a_i := ifelse(Prob_chosen_2a_i >= logistic_cutoff, 1, 0)]
datSICR_valid[, Pred_chosen_2a_i := ifelse(Prob_chosen_2a_i >= logistic_cutoff, 1, 0)]
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_2a_i, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_2a_i, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
...
datSICR_valid[, SICR_predict_variance := sd(Prob_chosen_2a_i), by=list(LoanID)]
standard_deviation <- round(mean(datSICR_valid$SICR_predict_variance, na.rm=T)*100, digits=1)

# - Confusion matrix
conf_mat <- datSICR_valid[, list(TN=sum(ifelse(SICR_target == 0 & Pred_chosen_2a_i == 0, 1, 0)),
                                 FP=sum(ifelse(SICR_target == 0 & Pred_chosen_2a_i == 1, 1, 0)),
                                 TP=sum(ifelse(SICR_target == 1 & Pred_chosen_2a_i == 1, 1, 0)),
                                 FN=sum(ifelse(SICR_target == 1 & Pred_chosen_2a_i == 0, 1, 0)))]
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.1808534

# A few things of concern:
# 1) Volatility in event rates due to relatively low sampling volumes in validation set
# 2) 0-counts over 20006-2007 periods using discretised output [Pred_chosen_2a_i]
...
performance_measures_2a_i <- data.frame(SICR_definition = paste0(SICR_label, "_logit"),
                                        d=p.d, s=p.s, k=p.k,
...
pack.ffdf(paste0(genObjPath, "performance_measures_", SICR_label), performance_measures_2a_i); gc()
