...
summary(logit_model_chosen)
# Without the inclusion of PD-ratio, not all the variables are statistically significant
# PD-ratio is not statistically significant with a p-value of 0.899381 and a standard error of 0.000026964
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_2b_iii) # 90.30% vs 90.20%
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_2b_iii, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_2b_iii, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
...
pack.ffdf(paste0(genPath, "performance_measures_", SICR_label), performance_measures_2b_iii); gc()
