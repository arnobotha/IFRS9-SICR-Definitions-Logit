...
# PD ratio is not statistically significant (p-value of 0.370771 and standard error of 0.0009790)

# The following variables are insignificant:
# Term (p-value of 13.61%), TimeInPerfSpell (p-value of 68.84%) and M_Inflation_Growth (p-value of 17.89%)
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_1c_iv, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_1c_iv, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
...
pack.ffdf(paste0(genPath, "performance_measures_", SICR_label), performance_measures_1c_iv); gc()
