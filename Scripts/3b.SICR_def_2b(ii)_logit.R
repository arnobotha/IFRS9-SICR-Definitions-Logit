# ============================== SICR-MODELLIING ===============================
# SICR-model for definition 2b(ii) using Logistic Regression (LR)
# ------------------------------------------------------------------------------
...
ggplot(plot.data_SICR_rates, aes(x=Date, y=Proportion, colour=Prop)) + 
  ...
  labs(y="SICR-incidence rates for definition with parameters k=6, d=2, and s=2", x= "Time") + 
...
# Age_Adj, Term, FurtherLoan_Ind, FurtherLoan_Amt, AgeToTerm, BalanceToTerm
...
# Age_Adj, Term and AgeToTerm

# - Remove the insignificant variables from the "raw" variable theme and analyse again
inputs_account3 <- SICR_target_event ~ Receipt_Inf + Balance + Redraw_Ind + Redrawn_Amt + InterestRate_Margin + LN_TPE
...
inputs_account4 <- SICR_target_event ~ Receipt_InfLog + BalanceLog + LN_TPE + BalanceToTerm + InterestRate_Margin
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_3) # 62.45%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_3) # 65.56%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_4) # 65.78%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_4) # 67.49%
...
# PerfSpell_Num is insignificant
...
# Since this variables is significant in the other definitions, keep until all the variables are investigated together
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_1) # 88.96%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_1) # 90.25%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_2) # 88.96%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_2) # 90.24%

summary(logit_model_delinq1)
summary(logit_model_delinq2)
# Keep the imputed variables since there is almost no difference in the AUC-values and the same number of variables are insignificant for both
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_3) # 74.35% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_3) # 73.92%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_4) # 74.35% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_4) # 73.92%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_1) # 58.60% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_1) # 58.40%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_2) # 58.53% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_2) # 58.29%

# Keep the zero-month lag as a minimum
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_3) # 59.90% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_3) # 59.66%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_4) # 59.88% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_4) # 59.68%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_5) # 60.85% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_5) # 60.89%
...
# M_DTI_Growth_12 is the most significant (13.40% p-value)
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_6) # 60.91% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_6) # 60.97%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_7) # 59.87% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_7) # 59.61%
...
# M_Emp_Growth_12 is the most significant (p-value of 15.20%)
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_8) # 59.51% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_8) # 58.99%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_9) # 60.77% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_9) # 60.74%
...
# M_RealGDP_Growth_12 is the most significant (p-value of 10.40%)
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_10) # 59.82% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_10) # 59.59%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_11) # 61.02% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_11) # 60.99%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_12) # 60.13% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_12) # 59.79%

# The reduction in AUC is more significant than the previous variables, but the standard errors are big for the volatility variables

# Model with all the significant MVs
inputs_macro7 <- SICR_target_event ~ M_Repo_Rate + M_Inflation_Growth + M_DTI_Growth + M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_12 + M_RealIncome_Growth + 
                                     M_RealIncome_Growth_12 + M_RealGDP_Growth + M_RealGDP_Growth_12


# --- 4.5 Combined information
# Analyse the all the significant variables from the modelling themes
inputs_chosen <- SICR_target_event ~ Receipt_InfLog + BalanceLog + InterestRate_Margin + 
                                     PerfSpell_Num + g0_Delinq + PrevDefaults + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed +
                                     slc_pmnt_method + slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Inflation_Growth + 
                                     M_DTI_Growth + M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_12 + M_RealIncome_Growth + 
                                     M_RealIncome_Growth_12 + M_RealGDP_Growth + M_RealGDP_Growth_12
                                     # LN_TPE + BalanceToTerm + TimeInPerfSpell
...
# LN_TPE, BalanceToTerm and TimeInPerfSpell are insignificant
# The most significant MVs are:
# M_DTI_Growth, M_RealGDP_Growth_12, M_Repo_Rate and M_Emp_Growth_12
# Iteration 2
# Some of the MVs are still insignificant, but will analyse on the smaller dataset
# Inflation growth rate is insignificant as well as the 0-month lag of real GDP growth - the remaining have p-values smaller than 40%
# PerfSpell_Num is rather significant (p-value of 15.87%), two groups of payment method are insignificant

# - Compute the AUC
dat_SICR_def_train_s[, Prob_chosen_2b_ii := predict(logit_model_chosen, newdata = dat_SICR_def_train_s, type="response")] 
dat_SICR_def_valid_s[, Prob_chosen_2b_ii := predict(logit_model_chosen, newdata = dat_SICR_def_valid_s, type="response")]

auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_chosen_2b_ii) 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_chosen_2b_ii) 

# Report the AUC for the validation dataset
# Iteration 1's AUC: 91.25% | Iteration 2's AUC: 91.24%

# - Include only the numerical variables to analyse possible multicollinearity
inputs_all_mc1 <- SICR_target_event ~ Receipt_InfLog + BalanceLog + InterestRate_Margin + 
                                      PerfSpell_Num + g0_Delinq + PrevDefaults + slc_acct_roll_ever_24_imputed +
...
# Only the MVs have VIF > 10, but this is expected

# - Final variable selection
# After the initial feature selection across the 1million observations sample across 2b(i)-(iv), the following variables have been identified as significant
# across the definitions:
# Age_Adj, InterestRate_Margin, BalanceToTerm, BalanceLog, Receipt_InfLog, slc_pmnt_method, slc_acct_pre_lim_perc_imputed,
# TimeInPerfSpell, PerfSpell_Num, g0_Delinq, PrevDefaults, slc_acct_arr_dir_3, slc_acct_roll_ever_24_imputed,
# M_Repo_Rate, M_Inflation_Growth, M_DTI_Growth, M_DTI_Growth_12, M_Emp_Growth, M_Emp_Growth_12, M_RealGDP_Growth, 
# M_RealGDP_Growth_12, M_RealIncome_Growth and M_RealIncome_Growth_12

# Now, the input space should be finalized across definitions 2b(i)-(iv)
...
# Choose between the age variable or balance to term (both are age-related variables)
# Age_Adj
inputs_acc_level <- SICR_target_event ~ Age_Adj + InterestRate_Margin + BalanceLog + Receipt_InfLog
...
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_acc_level) # 65.94%

# BalanceToTerm
inputs_acc_level2 <- SICR_target_event ~ BalanceToTerm + InterestRate_Margin + BalanceLog + Receipt_InfLog
...
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_acc_level2) # 67.19%
# BalanceToTerm is more significant

# Choose between the balance and receipts variable
# BalanceLog
inputs_acc_level3 <- SICR_target_event ~  BalanceToTerm + InterestRate_Margin + BalanceLog
...
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_acc_level3) # 61.53%

# Receipt_InfLog
inputs_acc_level4 <- SICR_target_event ~  BalanceToTerm + InterestRate_Margin + Receipt_InfLog
logit_model_acc_level4 <- glm(inputs_acc_level4, data=dat_SICR_def_train_s, family="binomial")
summary(logit_model_acc_level4)
# All the variables are significant
dat_SICR_def_valid_s[, Prob_acc_level4 := predict(logit_model_acc_level4, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_acc_level4) # 64.11%
# Receipt_InfLog is more significant

# Delinquency-related variables
# With TimeInPerfSpell
inputs_del_level1 <- SICR_target_event ~  BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                          slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + PerfSpell_Num + g0_Delinq +
                                          PrevDefaults + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed
logit_model_del_level1 <- glm(inputs_del_level1, data=dat_SICR_def_train_s, family="binomial")
summary(logit_model_del_level1)
dat_SICR_def_valid_s[, Prob_del_level1 := predict(logit_model_del_level1, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_del_level1) # 90.84%

# Without TimeInPerfSpell
inputs_del_level2 <- SICR_target_event ~  BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                          slc_acct_pre_lim_perc_imputed + PerfSpell_Num + g0_Delinq +
                                          PrevDefaults + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed
logit_model_del_level2 <- glm(inputs_del_level2, data=dat_SICR_def_train_s, family="binomial")
summary(logit_model_del_level2)
dat_SICR_def_valid_s[, Prob_del_level2 := predict(logit_model_del_level2, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_del_level2) # 90.68%
# TimeInPerfSpell is significant - keep

# Without PerfSpell_Num
inputs_del_level3 <- SICR_target_event ~  BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                          slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + g0_Delinq +
                                          PrevDefaults + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed
logit_model_del_level3 <- glm(inputs_del_level3, data=dat_SICR_def_train_s, family="binomial")
summary(logit_model_del_level3)
dat_SICR_def_valid_s[, Prob_del_level3 := predict(logit_model_del_level3, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_del_level3) # 90.85%
# PerfSpell_Num is not significant - remove

# Without PrevDefaults
inputs_del_level4 <- SICR_target_event ~  BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                          slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + g0_Delinq + slc_acct_arr_dir_3 + 
                                          slc_acct_roll_ever_24_imputed
logit_model_del_level4 <- glm(inputs_del_level4, data=dat_SICR_def_train_s, family="binomial")
summary(logit_model_del_level4)
dat_SICR_def_valid_s[, Prob_del_level4 := predict(logit_model_del_level4, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_del_level4) # 91.08%
# PrevDefaults is not significant - remove

# All the possible macroeconomic variables
# Keep repo, inflation and DTI growth
# Test employment growth
inputs_macro_all <- SICR_target_event ~ BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                        slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + g0_Delinq + slc_acct_arr_dir_3 + 
                                        slc_acct_roll_ever_24_imputed + M_Repo_Rate + M_Inflation_Growth + M_DTI_Growth +
                                        M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_12
logit_macro_all <- glm(inputs_macro_all, data=dat_SICR_def_train_s, family="binomial")
summary(logit_macro_all)
# M_Emp_Growth is rather significant (p-value of 15.91%) and M_Emp_Growth_12 has a p-value of 24.41%
dat_SICR_def_valid_s[, Prob_macro_all := predict(logit_macro_all, newdata = dat_SICR_def_valid_s, type="response")]
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all) # 91.60%
# Remove the 12-month lag

# M_RealGDP_Growth and M_RealGDP_Growth_12
inputs_macro_all2 <- SICR_target_event ~ BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                         slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + g0_Delinq + slc_acct_arr_dir_3 + 
                                         slc_acct_roll_ever_24_imputed + M_Repo_Rate + M_Inflation_Growth + M_DTI_Growth +
                                         M_DTI_Growth_12 + M_Emp_Growth + M_RealGDP_Growth + M_RealGDP_Growth_12
...
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all2) # 91.57%
# Reduction in AUC - do not keep

# M_RealIncome_Growth and M_RealIncome_Growth_12
inputs_macro_all3 <- SICR_target_event ~ BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                         slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + g0_Delinq + slc_acct_arr_dir_3 + 
                                         slc_acct_roll_ever_24_imputed + M_Repo_Rate + M_Inflation_Growth + M_DTI_Growth +
                                         M_DTI_Growth_12 + M_Emp_Growth + M_RealIncome_Growth + M_RealIncome_Growth_12
...
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_all3) # 91.55%
# Reduction in AUC - do not keep
...
# Choose between the age variable or balance to term (both are age-related variables)
# Age_Adj
inputs_acc_level <- SICR_target ~ Age_Adj + InterestRate_Margin + BalanceLog + Receipt_InfLog
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_acc_level) # 64.54%

# BalanceToTerm
inputs_acc_level2 <- SICR_target ~ BalanceToTerm + InterestRate_Margin + BalanceLog + Receipt_InfLog
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_acc_level2) # 65.82%
# BalanceToTerm is more significant

# Choose between the balance and receipts variable
# BalanceLog
inputs_acc_level3 <- SICR_target ~  BalanceToTerm + InterestRate_Margin + BalanceLog
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_acc_level3) # 60.94%

# Receipt_InfLog
inputs_acc_level4 <- SICR_target ~  BalanceToTerm + InterestRate_Margin + Receipt_InfLog
logit_model_acc_level4 <- glm(inputs_acc_level4, data=datSICR_train, family="binomial")
summary(logit_model_acc_level4)
# All the variables are significant
datSICR_valid[, Prob_acc_level4 := predict(logit_model_acc_level4, newdata = datSICR_valid, type="response")]
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_acc_level4) # 63.25%
# Receipt_InfLog is more significant

# Delinquency-related variables
# With TimeInPerfSpell
inputs_del_level1 <- SICR_target ~  BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                    slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + PerfSpell_Num + g0_Delinq +
                                    PrevDefaults + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed
logit_model_del_level1 <- glm(inputs_del_level1, data=datSICR_train, family="binomial")
summary(logit_model_del_level1)
datSICR_valid[, Prob_del_level1 := predict(logit_model_del_level1, newdata = datSICR_valid, type="response")]
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_del_level1) # 90.45%

# Without TimeInPerfSpell
inputs_del_level2 <- SICR_target ~  BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                    slc_acct_pre_lim_perc_imputed + PerfSpell_Num + g0_Delinq +
                                    PrevDefaults + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed
logit_model_del_level2 <- glm(inputs_del_level2, data=datSICR_train, family="binomial")
summary(logit_model_del_level2)
datSICR_valid[, Prob_del_level2 := predict(logit_model_del_level2, newdata = datSICR_valid, type="response")]
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_del_level2) # 90.39%
# TimeInPerfSpell is significant - keep

# Without PerfSpell_Num
inputs_del_level3 <- SICR_target ~  BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                    slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + g0_Delinq +
                                    PrevDefaults + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed
logit_model_del_level3 <- glm(inputs_del_level3, data=datSICR_train, family="binomial")
summary(logit_model_del_level3)
datSICR_valid[, Prob_del_level3 := predict(logit_model_del_level3, newdata = datSICR_valid, type="response")]
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_del_level3) # 90.45%
# PerfSpell_Num is not significant - remove

# Without PrevDefaults
inputs_del_level4 <- SICR_target ~  BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                    slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + g0_Delinq + slc_acct_arr_dir_3 + 
                                    slc_acct_roll_ever_24_imputed
logit_model_del_level4 <- glm(inputs_del_level4, data=datSICR_train, family="binomial")
summary(logit_model_del_level4)
datSICR_valid[, Prob_del_level4 := predict(logit_model_del_level4, newdata = datSICR_valid, type="response")]
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_del_level4) # 90.40%
# PrevDefaults is not significant - remove

# All the possible macroeconomic variables
# Keep repo, inflation and DTI growth
# Test employment growth
inputs_macro_all <- SICR_target ~ BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                  slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + g0_Delinq + slc_acct_arr_dir_3 + 
                                  slc_acct_roll_ever_24_imputed + M_Repo_Rate + M_Inflation_Growth + M_DTI_Growth +
                                  M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_12
logit_macro_all <- glm(inputs_macro_all, data=datSICR_train, family="binomial")
summary(logit_macro_all)
# M_Emp_Growth is rather significant (p-value of 11.92%) and M_Emp_Growth_12 has a p-value of 55.38%
datSICR_valid[, Prob_macro_all := predict(logit_macro_all, newdata = datSICR_valid, type="response")]
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all) # 90.35%
# Remove the 12-month lag

# M_RealGDP_Growth and M_RealGDP_Growth_12
inputs_macro_all2 <- SICR_target ~ BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                   slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + g0_Delinq + slc_acct_arr_dir_3 + 
                                   slc_acct_roll_ever_24_imputed + M_Repo_Rate + M_Inflation_Growth + M_DTI_Growth +
                                   M_DTI_Growth_12 + M_Emp_Growth + M_RealGDP_Growth + M_RealGDP_Growth_12
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all2) # 90.29%
# Reduction in AUC - do not keep

# M_RealIncome_Growth and M_RealIncome_Growth_12
inputs_macro_all3 <- SICR_target ~ BalanceToTerm + InterestRate_Margin + Receipt_InfLog + slc_pmnt_method + 
                                   slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + g0_Delinq + slc_acct_arr_dir_3 + 
                                   slc_acct_roll_ever_24_imputed + M_Repo_Rate + M_Inflation_Growth + M_DTI_Growth +
                                   M_DTI_Growth_12 + M_Emp_Growth + M_RealIncome_Growth + M_RealIncome_Growth_12
...
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_macro_all3) # 90.33%
# Reduction in AUC - do not keep
...
# - Check the event rate of each class | RECORD-LEVEL
table(datSICR$SICR_target) %>% prop.table() # 99.32% Non-SICR vs 0.68% SICR
...
# - Define model form
inputs_chosen <- SICR_target ~ BalanceToTerm + InterestRate_Margin + Receipt_InfLog + pmnt_method_grp + 
                               slc_acct_pre_lim_perc_imputed + TimeInPerfSpell + g0_Delinq + slc_acct_arr_dir_3 + 
                               slc_acct_roll_ever_24_imputed + M_Repo_Rate + M_Inflation_Growth + M_DTI_Growth +
...
datSICR_valid[, Prob_chosen_2b_ii := predict(logit_model_chosen, newdata = datSICR_valid, type="response")]
datSICR_smp[, ExpProb := predict(logit_model_chosen, newdata = datSICR_smp, type="response")]

# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_2b_ii) # 92.32%
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_2b_ii) # 90.58%
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 91.59%
...
ggplot( data=datSICR_valid, aes(x=Prob_chosen_2b_ii)) + theme_bw() + 
...
optimal.cutpoint.GenYouden <- optimal.cutpoints(X = "Prob_chosen_2b_ii", status = "SICR_target", tag.healthy = 0, # these are the negatives
...
datSICR_train[, Pred_chosen_2b_ii := ifelse(Prob_chosen_2b_ii >= logistic_cutoff, 1, 0)]
datSICR_valid[, Pred_chosen_2b_ii := ifelse(Prob_chosen_2b_ii >= logistic_cutoff, 1, 0)]
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_2b_ii, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_2b_ii, data=datSICR_valid, ci.method="bootstrap", ci=T, conf.level = 1-alpha, percent=T)
...
datSICR_valid[, SICR_predict_variance := sd(Prob_chosen_2b_ii), by=list(LoanID)]
standard_deviation <- round(mean(datSICR_valid$SICR_predict_variance, na.rm=T)*100, digits=1)

# - Confusion matrix
conf_mat <- datSICR_valid[, list(TN=sum(ifelse(SICR_target == 0 & Pred_chosen_2b_ii == 0, 1, 0)),
                                 FP=sum(ifelse(SICR_target == 0 & Pred_chosen_2b_ii == 1, 1, 0)),
                                 TP=sum(ifelse(SICR_target == 1 & Pred_chosen_2b_ii == 1, 1, 0)),
                                 FN=sum(ifelse(SICR_target == 1 & Pred_chosen_2b_ii == 0, 1, 0)))]
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.1613168

# A few things of concern:
# 1) Volatility in event rates due to relatively low sampling volumes in validation set
# 2) 0-counts over 20006-2007 periods using discretised output [Pred_chosen_2b_ii]
...
performance_measures_2b_ii <- data.frame(SICR_definition = paste0(SICR_label, "_logit"),
                                         d=p.d, s=p.s, k=p.k,
...
pack.ffdf(paste0(genPath, "performance_measures_", SICR_label), performance_measures_2b_ii); gc()
