...
summary(logit_model_chosen)
# Most variables are statistically significant, except: slc_acct_arr_dir_3 (all bins)
# [Ad hoc] PD ratio is not statistically significant (p-value of 0.95064 and standard error of 0.000119896)
...
# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_2b_ii) # 92.16%
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_2b_ii) # 90.49%
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 91.47%
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_2b_ii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_2b_ii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.7751936
...
pack.ffdf(paste0(genObjPath, "performance_measures_", SICR_label), performance_measures_2b_ii); gc()
