...
if(!exists('datCredit_allInputs')) unpack.ffdf(paste0(genPath,"creditdata_allinputs"), tempPath)
datSICR <- copy(datCredit_allInputs)
...
# - Define model form
inputs_chosen <- SICR_target ~ Term + InterestRate_Margin + BalanceToTerm + TimeInPerfSpell + PerfSpell_Num + g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed +
                               slc_acct_pre_lim_perc_imputed + pmnt_method_grp + M_Repo_Rate + M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth + M_Inflation_Growth
...
# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_1b_ii) # 90.03%
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_1b_ii) # 89.41%
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 89.84%
...
optimal.cutpoint.GenYouden <- optimal.cutpoints(X = "Prob_chosen_1b_ii", status = "SICR_target", tag.healthy = 0, # these are the negatives
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.11299069
