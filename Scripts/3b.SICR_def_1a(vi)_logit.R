...
# Number of impacted observations: 31.10%
...
ggplot(plot.data_SICR_rates, aes(x=Date, y=Proportion, colour=Prop)) + 
  ...
  labs(y="SICR-incidence rates for definition with parameters k=24, d=1, and s=1", x= "Time") + 
...
# Redraw_Ind (so we need to remove Redraw_Amt as well), FurtherLoan_Amt (so remove FurtherLoan_Ind too), AgeToTerm and BalanceToTerm
...
inputs_account3 <- SICR_target_event ~ Age_Adj + Term + Receipt_Inf + Balance + InterestRate_Margin + LN_TPE
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_2) # 63.55%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_2) # 63.59%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_account_3) # 59.89%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_account_3) # 59.83%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_1) # 77.35%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_1) # 77.44%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_delinq_2) # 77.35%
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_delinq_2) # 77.43%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_3) # 71.38% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_3) # 71.37%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_behav_4) # 71.38% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_behav_4) # 71.37%
...
# M_Repo_Rate_1, M_Repo_Rate_11, M_Repo_Rate_12 and M_Repo_Rate_Vol_12 are significant
...
# - Compute the AUC to investigate the difference between using the volatility variables and the raw variables
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_1) # 54.83% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_1) # 54.82%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_2) # 53.10% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_2) # 53.11%

# RESULT: Less than a 2% reduction in AUC
...
# M_Inflation_Growth, M_Inflation_Growth_7, M_Inflation_Growth_10, M_Inflation_Growth_12, 
# M_Inflation_Growth_Vol_7, M_Inflation_Growth_Vol_8, M_Inflation_Growth_Vol_9 and M_Inflation_Growth_Vol_12 are significant
...
# M_Inflation_Growth, M_Inflation_Growth_7 and M_Inflation_Growth_10 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_3) # 53.97% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_3) # 53.97%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_4) # 53.84% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_4) # 53.85%
...
# M_DTI_Growth, M_DTI_Growth_1, M_DTI_Growth_6, M_DTI_Growth_12, M_DTI_Growth_Vol_2, M_DTI_Growth_Vol_7, M_DTI_Growth_Vol_8,
# M_DTI_Growth_Vol_11 and M_DTI_Growth_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_5) # 56.13% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_5) # 56.11%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_6) # 55.77% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_6) # 55.76%

# RESULT: Around a 0.5% reduction in AUC
...
# M_Emp_Growth, M_Emp_Growth_1, M_Emp_Growth_11, M_Emp_Growth_12, M_Emp_Growth_Vol_7, M_Emp_Growth_Vol_10 and M_Emp_Growth_Vol_12 are significant
...
# M_Emp_Growth, M_Emp_Growth_1 and M_Emp_Growth_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_7) # 55.62% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_7) # 55.61%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_8) # 54.74% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_8) # 54.75%
...
# M_RealGDP_Growth, M_RealGDP_Growth_Vol_2, M_RealGDP_Growth_Vol_11 and M_RealGDP_Growth_Vol_12 are significant
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_9) # 56.11% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_9) # 56.09%
...
auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_macro_10) # 55.78% 
auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_macro_10) # 55.77%

# RESULT: Less than a 0.5% reduction in AUC
...
# M_RealIncome_Growth, M_RealIncome_Growth_12 and M_RealIncome_Growth_Vol_12 are significant
...
inputs_macro <- SICR_target_event ~ M_Repo_Rate + M_Repo_Rate_2 + M_Repo_Rate_12 + M_Inflation_Growth + M_Inflation_Growth_7 + M_Inflation_Growth_10 +
                                    M_DTI_Growth + M_DTI_Growth_12 + M_Emp_Growth + M_Emp_Growth_1 + M_Emp_Growth_12 + M_RealGDP_Growth +
...
# inputs_chosen <- SICR_target_event ~ Term + Receipt_InfLog + BalanceLog + LN_TPE + BalanceToTerm + InterestRate_Margin +
                                     ...
                                     # M_Repo_Rate + M_Repo_Rate_12 + M_Inflation_Growth + M_Inflation_Growth_10 +
                                     # M_DTI_Growth + M_DTI_Growth_12 + M_Emp_Growth + M_RealGDP_Growth +
                                     # M_RealGDP_Growth_12 + M_RealIncome_Growth + M_RealIncome_Growth_12 
                                     # AgeToTerm + M_Emp_Growth_12 + M_Repo_Rate_2 + M_Inflation_Growth_7 + M_Emp_Growth_1 + Age_Adj
                                     
# logit_model_chosen <- glm(inputs_chosen, data=dat_SICR_def_train_s, family="binomial")
# summary(logit_model_chosen)

# Note: The suspense payment method is insignificant

# - Compute the AUC
# dat_SICR_def_train_s[, Prob_chosen_1a_vii := predict(logit_model_chosen, newdata = dat_SICR_def_train_s, type="response")] 
# dat_SICR_def_valid_s[, Prob_chosen_1a_vii := predict(logit_model_chosen, newdata = dat_SICR_def_valid_s, type="response")]

# auc(dat_SICR_def_train_s$SICR_target_event, dat_SICR_def_train_s$Prob_chosen_1a_vii) 
# auc(dat_SICR_def_valid_s$SICR_target_event, dat_SICR_def_valid_s$Prob_chosen_1a_vii) 

# Report the AUC for the validation dataset
# Iteration 1's AUC: 80.96% | Iteration 2's AUC: 80.96% | Iteration 3's AUC: 80.94% | Iteration 4's AUC: 80.94% | Iteration 5's AUC: 80.93%

# Note: With the volatility variables - Report the AUC for the validation dataset
# Iteration 1's AUC: 80.99% | Iteration 2's AUC: 80.97% | Iteration 3's AUC: 80.96% | Iteration 4's AUC: 80.91% | Iteration 5's AUC: 80.91%
# Iteration 6's AUC: 80.91%
...
# Iteration 1: Age_Adj and TimeInPerfSpell and all the macroeconomic variables (to be expected)
# Remove Age_Adj
# Iteration 2: Macroeconomic variables (to be expected)
...
table(datSICR$SICR_target) %>% prop.table() # 94.51% Non-SICR vs 5.49% SICR
...
datSICR_valid[, Prob_chosen_1a_vi := predict(logit_model_chosen, newdata = datSICR_valid, type="response")]
datSICR_smp[, ExpProb := predict(logit_model_chosen, newdata = datSICR_smp, type="response")]

# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_1a_vi) # 80.34%
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_1a_vi) # 80.63%
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 80.43%
...
# --- 5.2 Find optimal points (c) according to several measures using the validation data
...
optimal.cutpoint.GenYouden <- optimal.cutpoints(X = "Prob_chosen_1a_vi", status = "SICR_target", tag.healthy = 0, # these are the negatives
...
datSICR_train[, Pred_chosen_1a_vi := ifelse(Prob_chosen_1a_vi >= logistic_cutoff, 1, 0)]
datSICR_valid[, Pred_chosen_1a_vi := ifelse(Prob_chosen_1a_vi >= logistic_cutoff, 1, 0)]
...
datSICR_valid[, SICR_predict_variance := sd(Prob_chosen_1a_vi), by=list(LoanID)]
standard_deviation <- round(mean(datSICR_valid$SICR_predict_variance, na.rm=T)*100, digits=1)

# - Confusion matrix
conf_mat <- datSICR_valid[, list(TN=sum(ifelse(SICR_target == 0 & Pred_chosen_1a_vi == 0, 1, 0)),
                                 FP=sum(ifelse(SICR_target == 0 & Pred_chosen_1a_vi == 1, 1, 0)),
                                 TP=sum(ifelse(SICR_target == 1 & Pred_chosen_1a_vi == 1, 1, 0)),
                                 FN=sum(ifelse(SICR_target == 1 & Pred_chosen_1a_vi == 0, 1, 0)))]
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.11534144
...
performance_measures_1a_vi <- data.frame(SICR_definition = paste0(SICR_label, "_logit"),
                                         d=p.d, s=p.s, k=p.k,
