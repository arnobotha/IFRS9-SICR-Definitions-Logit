...
# TimeInPerfSpell (p-value of 27.03%) and PerfSpell_Num (p-value of 10.25%), as well as inflation growth (p-value of 27.01%)
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_1c_iii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_1c_iii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
...
pack.ffdf(paste0(genObjPath, "performance_measures_", SICR_label), performance_measures_1c_iii); gc()
