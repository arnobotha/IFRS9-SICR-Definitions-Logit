...
table(datSICR$SICR_target) %>% prop.table() # 93.85% Non-SICR vs 6.15% SICR
...
pROC_obj_chosena <- roc(formula= SICR_target~Pred_chosen_1a_ii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
pROC_obj_chosenb <- roc(formula= SICR_target~Prob_chosen_1a_ii, data=datSICR_valid, ci.method="delong", ci=T, conf.level = 1-alpha, percent=T)
...
pack.ffdf(paste0(genObjPath, "performance_measures_", SICR_label), performance_measures_1a_ii); gc()
