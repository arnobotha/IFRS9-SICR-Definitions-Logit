...
# - Define model form
inputs_chosen <- SICR_target ~ Term + BalanceLog + InterestRate_Margin + TimeInPerfSpell + PerfSpell_Num +
                               g0_Delinq + slc_acct_arr_dir_3 + slc_acct_roll_ever_24_imputed + pmnt_method_grp +
                               slc_acct_pre_lim_perc_imputed + M_Repo_Rate + M_Inflation_Growth + 
                               M_DTI_Growth + M_DTI_Growth_12 + M_RealIncome_Growth
...
# - Compute the AUC
auc(datSICR_train$SICR_target, datSICR_train$Prob_chosen_1c_ii) # 91.09%
auc(datSICR_valid$SICR_target, datSICR_valid$Prob_chosen_1c_ii) # 91.55%
auc(datSICR_smp$SICR_target, datSICR_smp$ExpProb) # 91.23%
...
optimal.cutpoint.GenYouden <- optimal.cutpoints(X = "Prob_chosen_1c_ii", status = "SICR_target", tag.healthy = 0, # these are the negatives
...
if (!exists('logistic_cutoff')) logistic_cutoff <- 0.09225734
